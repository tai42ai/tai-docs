...
``None``; ``notify`` sends a fire-and-forget message the same way and returns the
per-message ids the medium assigned, which correlate a later delivery receipt back
to this send. Every failure raises ``ChannelDeliveryError`` — neither method returns
a bool, so an undeliverable question cannot read as a silent pass while the asking
tool blocks on an answer that would never arrive. Credentials and the recipient come
from the environment, never from tool parameters, so no secret or address rides an
LLM-visible value.
...
class EchoChannel:
    ...
    async def _send(self, text: str, recipient: str | None, context: str) -> list[str]:
        ...
        # The medium's assigned id for this send; an out-of-band receipt later
        # correlates against it. A medium exposing no id returns an empty list.
    ...
    async def notify(self, notification: ChannelNotification) -> list[str]:
        return await self._send(notification.message, notification.recipient, "notification")
