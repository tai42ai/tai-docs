...
- **`tool`** (`target_name` a registered tool) — the turn dispatches the tool
  **statelessly**: no thread, no conversation memory, one message in and one reply
...
- **`payload_expr`** — jq over the inbound payload `{message, sender, our_identity,
  channel}`, emitting exactly one JSON object: the tool's kwargs. Omitted, the kwargs
  are the fixed `{message, sender}`.
...
| `client_address` | The address the thread belongs to. |
...
reads an API-door thread its own principal keys — the thread id is
`bridge:{route_name}:{percent-encoded principal}/{external user id}` there, so a
thread names its owner. A channel thread's addresses are the medium's and belong to
