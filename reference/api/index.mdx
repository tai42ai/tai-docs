...
A validation failure names **which** fields failed and **how**, never **what** you
sent. The body lists each offending field's path and its error type only — the
rejected input values are stripped — so a secret submitted in a rejected field can
never ride back out in the error response.
