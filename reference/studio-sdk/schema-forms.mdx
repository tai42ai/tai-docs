...
```ts
type ExpressionFieldComponent = ComponentType<ExpressionFieldProps>
```
...
**Related:** [ExpressionFieldProps](/reference/studio-sdk/schema-forms#expressionfieldprops)


## ExpressionFieldContext

```ts
const ExpressionFieldContext: Context<ExpressionFieldComponent | undefined>
```
...
**Related:** [ExpressionFieldComponent](/reference/studio-sdk/schema-forms#expressionfieldcomponent)
...
| `error?` | `ReactNode` | — |
| `label` | `string` | — |
| `multiline?` | `boolean` | Render a multiline resting control; the form always asks for one. |
| `onChange` | `(value: string) => void` | — |
| `shape?` | `ExpressionInputShape` | — |
| `value` | `string` | — |

**Related:** [ExpressionInputShape](/reference/studio-sdk/schema-forms#expressioninputshape)
...
```ts
interface ExpressionInputShape
```
...
| Property | Type | Description |
|---|---|---|
| `blurb` | `string` | One sentence: what `.` is in this field. |
| `caveats?` | `readonly string[]` | Per-field caveats. |
| `id` | `string` | Stable, host-namespaced id, opaque to the door (memoisation/telemetry). |
| `keys` | `readonly ExpressionInputKey[]` | The top-level keys of `.`, each with a one-liner. |
| `label` | `string` | Short chip label — what `.` is here, e.g. "node envelope". |
| `returns` | `string` | What the expression must RETURN, e.g. "true or false". |
| `sample?` | `unknown` | A static skeleton of `.` for the door's test surface. |
| `variables?` | `readonly ExpressionInputVariable[]` | The named variables the expression reads as `$name` beside `.`; omitted when  the field declares none. |

**Related:** [ExpressionInputKey](/reference/studio-sdk/schema-forms#expressioninputkey)
...
| `expressionField?` | `ExpressionFieldComponent` | Optional. The expression-authoring door for `x-tai42-expression` fields — `JqField` from `@tai42/jq-studio`, which the host imports directly and passes here (see the module doc-comment). Omitted, the enclosing ExpressionFieldContext supplies it; absent both, an annotated field renders the plain string input. |
| `idPrefix?` | `string` | — |
...
**Related:** [CompletionProvider](/reference/studio-sdk/schema-forms#completionprovider), [ExpressionFieldComponent](/reference/studio-sdk/schema-forms#expressionfieldcomponent), [JsonSchema](/reference/studio-sdk/schema-forms#jsonschema)
...
| `idPrefix?` | `string` | — |
| `initialMode?` | `"key" \| "paste"` | Which editor a value-less field opens on when key picking is available. Defaults to `'paste'` (the write-first behaviour): a host adding a brand-new secret lands on the paste input. A host that only ever REFERENCES existing keys passes `'key'` so a fresh field opens straight on the key picker rather than a paste input it would block. Has no effect once a value is committed (the chip is shown) or when key picking is off (paste-only, fail closed). |
