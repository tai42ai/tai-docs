...
| `submitDisabled?` | `boolean` | Blocks submission — by click AND by Enter — while the form is incomplete. A disabled default button is not an implicit-submission target, so the keyboard path is closed off along with the pointer one. |
