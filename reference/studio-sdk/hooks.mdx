...
| `disabled` | `boolean` | The interactions store is not configured on this deployment: the stream answered with a terminal 501 `interactions-not-configured`. Reconnection is abandoned and the consumer renders the muted OFF state instead of a red error. |
| `error` | `Error \| null` | — |
...
function coversAnyRoute(
  projection: { admin: boolean; agents: string[]; mintable: boolean; owner_user_id: string | null; principal?: { display_name: string; kind: "human" | "service"; user_id: string } | null; route_patterns: ({ pattern: string; scope_id: string })[]; routes: ({ methods: string[]; path: string })[]; scopes: string[]; sub_mcp: ({ slug: string; tools: string[]; transport: string })[]; tools: string[]; user_id: string },
