...
| `seed` | `readonly ({ answer_format: "form" \| "select" \| "external" \| "text" \| "confirm"; audience?: string; channel?: string; created_at: string; format_payload: Record<string, unknown>; group_id: string; interaction_id: string; media?: unknown[]; origin?: string; question: string; recipient?: string; sensitive: boolean; server_verified?: boolean; timeout_at: string })[]` | The pending set from the paged `GET /api/interactions` door — the base the live tail overlays. A stable reference between refetches (memoize it upstream); a new reference re-merges but does NOT reopen the stream. |
