...
| `plugins?` | `readonly LoadedPlugin[]` | Identity (name + version) of every successfully-loaded plugin. Parallel to `loaded`; OPTIONAL and ADDITIVE, so an older host that never populated it still satisfies this shape (a reader treats absent as `[]`). The host reads it to attribute a contribution to its plugin — e.g. the nav provenance badge's "Plugin: name version". |
