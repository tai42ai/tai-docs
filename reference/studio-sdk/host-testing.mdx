...
| `plugins` | `readonly LoadedPlugin[]` | Identity (name + version) of every successfully-loaded plugin, parallel to `loaded`. The host reads it to attribute a contribution to its plugin — e.g. the nav provenance badge's "Plugin: name version". Empty before any plugin loads. |
...
**Related:** [LoadedPlugin](/reference/studio-sdk/host-testing#loadedplugin)
...
| `overflowing` | `boolean` | — | true to report content larger than the box on `axis`. |
| `axis` | `"horizontal" \| "vertical"` | `'horizontal'` | the dimension to fake: `horizontal` (width, the default) or   `vertical` (height). |
