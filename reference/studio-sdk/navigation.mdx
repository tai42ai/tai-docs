...
| `navigatePluginWithOptions?` | `(pluginId: string, pagePath: string, params?: string, search?: PluginSearch, options?: PluginNavigateOptions) => void` | ADDITIVE plugin-page navigation that also carries per-history-entry PluginNavigateOptions. OPTIONAL because a host that does not implement it leaves it undefined; plugins reach it — and require a host that provides it — through usePluginEntryNavigation, which throws loudly when it is absent. It behaves exactly like navigatePlugin for path / params / search, and in addition writes `options.state` onto the DESTINATION entry under the plugin's own namespace slot (see PluginPageProps.entryState).  It is a separate optional member rather than an extra parameter on navigatePlugin so it stays additive under the plugin-API equality gate: a new optional member does not move STUDIO_PLUGIN_API_VERSION, so every existing plugin keeps loading and navigatePlugin's own signature stays stable. |
...
| `conversations` | `{ address?: string; q?: string; route?: string; status?: "failed" \| "accepted" \| "pending_delivery" \| "provisional" \| "delivered" \| "shed" \| "silent" \| "merged" \| "superseded"; thread?: string }` | — |
