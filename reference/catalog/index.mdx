...
The catalog lists every registration shipped across the ecosystem — 42 entries across 12 kinds. Each row's repository is the package's home; follow it for that implementation's own documentation.
...
| `whatsapp-cloud` | chat | `tai42-channel-whatsapp-cloud` | tai-channel-whatsapp-cloud | Deliver ask_user questions to a human on WhatsApp via the Meta Cloud API and bridge the reply back. |
