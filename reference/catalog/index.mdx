...
The catalog lists every registration shipped across the ecosystem — 46 entries across 12 kinds. Each row's repository is the package's home; follow it for that implementation's own documentation.
...
| `confirm_stripe_payment` | payments | `tai42-toolbox` | tai-toolbox | Answer a payment ask from a projected checkout.session.completed event. |
| `create_stripe_checkout` | payments | `tai42-toolbox` | tai-toolbox | Create a Stripe Checkout Session and return its hosted payment URL. |
| `reconcile_stripe_payments` | payments | `tai42-toolbox` | tai-toolbox | Re-answer every paid Checkout Session in the lookback window the webhook path may have missed. |
