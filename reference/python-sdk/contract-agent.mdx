...
Implement `run`; override `astream` only when the agent produces real per-step
events.
...
```python
async append_thread_messages(self, *, thread_id: str, messages: list[dict[str, str]], **kwargs: Any) -> None
```
...
`tai42_contract.agent.events.RecursionLimitFinal`
...
| Attribute | Type |
|---|---|
| `type` | `Literal['recursion_limit_final']` |
| `limit` | `int` |
| `steps` | `int \| None` |
| `final` | `bool` |
...
Base class for every event an `Agent` streams.
...
`tai42_contract.agent.events.StructuredOutputUnresolvedFinal`
...
| Attribute | Type |
|---|---|
| `type` | `Literal['structured_output_unresolved_final']` |
| `schema_name` | `str` |
| `attempts` | `int` |
| `error` | `str` |
| `final` | `bool` |
...
| `name` | `str` |
| `description` | `str` |
| `system_prompt` | `TemplatedText \| None` |
| `tool_names` | `list[str]` |
| `tools` | `list[Any]` |
| `presets` | `list[PresetSpec]` |
| `skills` | `list[str]` |
| `inline_skills` | `list[dict[str, Any]]` |
| `response_format` | `type[BaseModel] \| TemplatedText \| dict[str, Any] \| None` |
| `strategy` | `str \| None` |
...
| Attribute | Type |
|---|---|
| `type` | `Literal['suspended_final']` |
| `interaction_ids` | `list[str]` |
| `thread_id` | `str` |
| `caller_interaction_ids` | `list[str]` |
| `expiry_at` | `str \| None` |
| `final` | `bool` |
...
The value one tool call returned, matched to a `ToolCallStep` by ``call_id``.
...
```python
agent_session_thread(thread_id: str) -> Generator[None]
```
...
```python
final_event_for_value(value: Any) -> MessageFinal | StructuredFinal
```

The terminal event a plain agent result value represents.

A ``str`` is a plain-text answer (`MessageFinal`); any other value is structured output
...
```python
get_agent_session_thread() -> str | None
```
...
```python
reset_agent_session_thread(token: Token[str | None]) -> None
```
...
```python
set_agent_session_thread(thread_id: str | None) -> Token[str | None]
```
