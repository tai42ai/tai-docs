...
`fetch_url` is re-exported LAZILY (PEP 562): it pulls in ``httpx``/``httpcore``,
and a consumer of a lighter ``net`` utility (e.g. `tai42_kit.net.request_body`)
must not drag that backend into its import graph merely by importing a sibling module.
This is the same reason `tai42_kit.net.jwt` (which needs ``joserfc``) stays out of
this re-export.
