...
`RUN_VERSION_METADATA_KEY` (not a discrete parameter) so a backend
...
```python
attribute_run(writer: MonitoringWriter, attribution: RunAttribution) -> AbstractContextManager[None]
```

Return the context manager that stamps ``attribution`` on the ambient
trace for the WRAPPED block.

A free function composing ``writer`` over `MonitoringWriter.trace_attributes`,
...
It does NOT gate on `MonitoringWriter.current_trace_id`. A stamp deposited
...
`MonitoringWriter.trace_attributes`'s (it catches + logs on enter/exit,
