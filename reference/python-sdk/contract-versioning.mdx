...
Raise `DocumentExistsError` if ``(kind, name)`` is
...
`DocumentNotFoundError` if absent. Runs within ``tx`` when one is supplied.
...
Raise `DocumentNotFoundError` if absent.
...
List every version of ``(kind, name)`` with its `DocumentVersion.is_current` signal.

``is_current`` is derived from ``active_version``. Raise `DocumentNotFoundError` if the
document is absent.
...
Raise `DocumentVersionNotFoundError` if that
...
Raise `DocumentVersionNotFoundError` if that version does not exist. Runs
...
HARD delete, DISTINCT from `soft_delete`.
