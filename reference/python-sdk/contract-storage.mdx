...
``FileNotFoundError`` and colliding paths to `StoragePathConflictError`.
...
— raises `StoragePathConflictError`. An id naming an empty leftover
...
Return the raw bytes at ``path``, with the same ``FileNotFoundError`` contract as `load`.
...
```python
conflicts_summary(self) -> str
```

The conflicting ids as a human string, capped at `_MESSAGE_CONFLICT_CAP`.
