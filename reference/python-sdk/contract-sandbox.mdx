...
Exposes the neutral session models, the resolved `SandboxPolicy`, the error family, and
the `Sandbox` / `SandboxSession` / `SandboxExecHandle` ABCs.
...
Create a session from ``spec`` or REJECT it with `SandboxSpecRejectedError`.
...
Raise `SandboxSessionNotFoundError` if absent.
...
The interleaved stdout/stderr stream, terminated by one `SandboxStreamExit`.
...
Start ``argv`` as an INTERACTIVE exec, returning a `SandboxExecHandle`.
...
Raise a typed `SandboxError` on a miss.
