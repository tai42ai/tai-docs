...
|---|---|
| `tool` | `str` |
| `condition` | `Annotated[str \| None, Field(json_schema_extra={EXPRESSION_ANNOTATION_KEY: expression_annotation(label='condition', blurb="the backend task's tool result", keys=[], returns='truthy to run the callback', sample={})})]` |
| `expr` | `Annotated[str \| None, Field(json_schema_extra={EXPRESSION_ANNOTATION_KEY: expression_annotation(label='expression', blurb="the backend task's tool result", keys=[], returns="an object — the follow-up tool's kwargs", caveats=['an empty expression, or a jq pipeline producing no output, yields {} (the follow-up tool runs with empty kwargs) — not an error'], sample={})})]` |
