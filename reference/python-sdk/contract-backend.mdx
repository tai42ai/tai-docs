...
```python
REGISTRY_MUTATING_FLEET_OPS: Final[frozenset[str]] = frozenset({'reload_config', 'reload_mcp', 'deregister_mcp', 'reload_tool', 'remove_tool', 'reload_failed_mcps'})
```
...
```python
TEMPLATE_EVICTION_FLEET_OPS: Final[frozenset[str]] = frozenset({'evict_template', 'clear_template_cache'})
```
...
```python
from_args(cls, args: Sequence[str]) -> Self
```
...
```python
async turn_over_pool(self, *, reason: str, budget: float) -> None
```
...
|---|---|
| `tool` | `str` |
| `condition` | `Annotated[TemplatedText \| None, Field(json_schema_extra={EXPRESSION_ANNOTATION_KEY: expression_annotation(label='condition', blurb="the backend task's tool result", keys=[], returns='truthy to run the callback', sample={})})]` |
| `expr` | `Annotated[TemplatedText \| None, Field(json_schema_extra={EXPRESSION_ANNOTATION_KEY: expression_annotation(label='expression', blurb="the backend task's tool result", keys=[], returns="an object — the follow-up tool's kwargs", caveats=['an empty expression, or a jq pipeline producing no output, yields {} (the follow-up tool runs with empty kwargs) — not an error'], sample={})})]` |
