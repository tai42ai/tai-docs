...
note to audio). ``location`` is a `LocationElement` the guest shared. Both
...
`tai42_contract.app.facets.AppStates.list_modules`

```python
async list_modules(self) -> list[StateModuleDocument]
```
...
`tai42_contract.app.facets.AppStates.get_module`

```python
async get_module(self, name: str) -> StateModuleDocument | None
```
...
`tai42_contract.app.facets.AppStates.put_module`

```python
async put_module(self, doc: StateModuleDocument, *, replace: bool) -> StateModuleDocument
```
...
| Parameter | Type | Default | Description |
|---|---|---|---|
| `doc` | `StateModuleDocument` | — | — |
| `replace` | `bool` | — | — |


#### delete_module

`tai42_contract.app.facets.AppStates.delete_module`

```python
async delete_module(self, name: str) -> None
```

Delete a module document; raise
`ModuleInUseError` while it is still mounted.
...
`tai42_contract.app.facets.AppStates.list_mounts`

```python
async list_mounts(self, state: str | None = None, *, module: str | None = None) -> list[dict[str, Any]]
```
...
`tai42_contract.app.facets.AppStates.mount`

```python
async mount(self, state: str, module: str, body: MountBody) -> None
```
...
`tai42_contract.app.facets.AppStates.update_mount_declarations`

```python
async update_mount_declarations(
    self,
    state: str,
    module: str,
...
`tai42_contract.app.facets.AppStates.unmount`

```python
async unmount(self, state: str, module: str) -> None
```
...
```python
async read(self, state: str, subject: StateSubject) -> RecordView | None
```
...
`tai42_contract.app.facets.AppStates.register_module_seed`

```python
register_module_seed(self, doc: StateModuleDocument) -> None
```
...
`tai42_contract.app.facets.AppStates.register_mount_validator`

```python
register_mount_validator(self, validator: MountValidator) -> None
```
...
`tai42_contract.app.facets.AppStates.register_mount_reconciler`

```python
register_mount_reconciler(self, reconciler: MountReconciler) -> None
```

Register a pre-write mount reconciler.

A plugin calls this through the ``tai42_app`` handle when its module loads. The
reconciler receives a `MountReconcileContext` — the
