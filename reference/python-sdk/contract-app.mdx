...
`CorrelationStore`, and an `InboundBridge` of the
...
`tai42_contract.app.facets.AppChannels.record_flow_send_receipt`

```python
async record_flow_send_receipt(
    self,
    channel: str,
...
(`validate_inbound_form`) and attaches no
...
inbound counterpart of an outbound answer's ``media`` — as `MediaItem`
(image/document/video/audio; a channel resolves a sticker to image/video, a voice
note to audio). ``location`` is a `LocationElement` the participant shared. Both
...
`tai42_contract.app.facets.AppConversations.register_target_validator`

```python
register_target_validator(self, target_kind: ConversationTargetKind, validator: TargetBindValidator) -> None
```
...
`tai42_contract.app.facets.AppHttp.use_raw_path_key`

```python
use_raw_path_key(self, path_prefix: str) -> None
```
...
`tai42_contract.app.facets.AppPresets.create`
...
`tai42_contract.app.facets.AppPresets.save_version`
...
`tai42_contract.app.facets.AppPresets.register_seed`

```python
register_seed(self, seed: PresetSeed) -> None
```
...
`tai42_contract.app.facets.AppStates`
...
The store takes a full `StateSubject` and refuses
anything less; subject resolution happens once per door, never here. Every write
door supplies a `WriteOrigin` carrying only what a
...
`StatesNotConfiguredError` (501), never an empty read.

### Members

#### list_declarations

`tai42_contract.app.facets.AppStates.list_declarations`

```python
async list_declarations(self) -> list[StateDeclaration]
```
...
`tai42_contract.app.facets.AppStates.get_declaration`

```python
async get_declaration(self, name: str) -> StateDeclaration | None
```
...
`tai42_contract.app.facets.AppStates.put_declaration`

```python
async put_declaration(self, decl: StateDeclaration) -> StateDeclaration
```
...
`NonAdditiveRedeclareError` (erase the records
first), and removing a subject kind still present in records raises
`DeclarationInUseError`. Never a silent overwrite.
...
`tai42_contract.app.facets.AppStates.delete_declaration`

```python
async delete_declaration(self, name: str) -> None
```

Delete a state with its records and attachments; raise
`DeclarationInUseError` when a consumer still
binds it.
...
`tai42_contract.app.facets.AppStates.stats`

```python
async stats(self, name: str) -> dict[str, Any]
```
...
`tai42_contract.app.facets.AppStates.list_templates`

```python
async list_templates(self) -> list[StateTemplateDocument]
```
...
`tai42_contract.app.facets.AppStates.get_template`

```python
async get_template(self, name: str) -> StateTemplateDocument | None
```
...
`tai42_contract.app.facets.AppStates.put_template`

```python
async put_template(self, doc: StateTemplateDocument, *, replace: bool) -> StateTemplateDocument
```
...
| Parameter | Type | Default | Description |
|---|---|---|---|
| `doc` | `StateTemplateDocument` | — | — |
| `replace` | `bool` | — | — |


#### delete_template

`tai42_contract.app.facets.AppStates.delete_template`

```python
async delete_template(self, name: str) -> None
```

Delete a template document; raise
`TemplateInUseError` while it is still attached.
...
`tai42_contract.app.facets.AppStates.list_attachments`

```python
async list_attachments(self, state: str | None = None, *, template: str | None = None) -> list[dict[str, Any]]
```
...
`tai42_contract.app.facets.AppStates.attach`

```python
async attach(self, state: str, template: str, body: AttachBody) -> None
```
...
`tai42_contract.app.facets.AppStates.update_attachment_declarations`

```python
async update_attachment_declarations(
    self,
    state: str,
    template: str,
...
`tai42_contract.app.facets.AppStates.detach`

```python
async detach(self, state: str, template: str) -> None
```
...
`tai42_contract.app.facets.AppStates.read`

```python
async read(self, state: str, subject: StateSubject) -> StateRecord | None
```
...
`tai42_contract.app.facets.AppStates.replace`
...
`tai42_contract.app.facets.AppStates.merge`
...
`tai42_contract.app.facets.AppStates.apply`
...
(`RegimeViolationError`) before the ledger insert,
...
`tai42_contract.app.facets.AppStates.eval_template_jq`

```python
async eval_template_jq(self, state: str, subject: StateSubject, name: str, params: dict[str, Any]) -> TemplateJqResult
```
...
template). An unknown name is a `StateNotFoundError`; an
ambiguous unqualified name, an ``update``-purpose name, an undeclared param or an
evaluation failure is a `ValueValidationError`. A read;
...
`tai42_contract.app.facets.AppStates.apply_template_jq`

```python
async apply_template_jq(
    self,
    state: str,
    subject: StateSubject,
    name: str,
...
path and applied through the same chokepoint as `apply` — so regimes, the
composing-shape guard, trace stamping and ``op_id`` idempotency all hold. ``name``
resolves as for `eval_template_jq`. An unknown name is a
`StateNotFoundError`; an ambiguous unqualified name, an
...
`tai42_contract.app.facets.AppStates.erase`

```python
async erase(self, state: str, subject: StateSubject, *, origin: WriteOrigin) -> None
```
...
`tai42_contract.app.facets.AppStates.fold`
...
schema raises `SubjectFoldError`; a retried fold to
...
`tai42_contract.app.facets.AppStates.list_subjects`
...
`tai42_contract.app.facets.AppStates.search`
...
`tai42_contract.app.facets.AppStates.writes`
...
`tai42_contract.app.facets.AppStates.prune_expired`

```python
async prune_expired(self) -> dict[str, int]
```
...
`tai42_contract.app.facets.AppStates.context`

```python
context(self) -> StateContext | None
```

The ambient `StateContext` the current door
...
`tai42_contract.app.facets.AppStates.register_consumer_lister`

```python
register_consumer_lister(self, kind: str, lister: ConsumerLister) -> None
```
...
|---|---|---|---|
| `kind` | `str` | — | — |
| `lister` | `ConsumerLister` | — | — |


#### consumers

`tai42_contract.app.facets.AppStates.consumers`

```python
async consumers(self, state: str) -> list[ConsumerRow]
```
...
`tai42_contract.app.facets.AppStates.register_template_seed`

```python
register_template_seed(self, doc: StateTemplateDocument) -> None
```
...
`tai42_contract.app.facets.AppStates.register_attach_validator`

```python
register_attach_validator(self, validator: AttachValidator) -> None
```
...
| Parameter | Type | Default | Description |
|---|---|---|---|
| `validator` | `AttachValidator` | — | — |


#### register_attach_reconciler

`tai42_contract.app.facets.AppStates.register_attach_reconciler`

```python
register_attach_reconciler(self, reconciler: AttachReconciler) -> None
```

Register a pre-write attach reconciler.

A plugin calls this through the ``tai42_app`` handle when its module loads. The
reconciler receives a `AttachReconcileContext` — the
...
`tai42_contract.app.facets.AppToolMeta`
...
``store`` is the `ToolMetaStore` view the routes
...
| Attribute | Type |
|---|---|
| `store` *(property)* | `ToolMetaStore` |

### Members

#### patch

`tai42_contract.app.facets.AppToolMeta.patch`
...
```python
register_delete_referee(self, provider: ToolDeleteReferee) -> None
```

Register a `ToolDeleteReferee` consulted before a preset delete.
...
```python
register_detach_referee(self, provider: StateTemplateDetachReferee) -> None
```

Register a `StateTemplateDetachReferee` consulted before a state-template
detach.
...
| Parameter | Type | Default | Description |
|---|---|---|---|
| `provider` | `StateTemplateDetachReferee` | — | — |
...
```python
register_tier(self, base_tool: str, tier: RouteAction) -> None
```

Declare ``base_tool``'s registration tier — the authorization character
(a `RouteAction`) enforced everywhere the tier is consulted.
...
```python
tier(self, base_tool: str) -> RouteAction | None
```
...
| `tool_meta` *(property)* | `AppToolMeta` |
| `states` *(property)* | `AppStates` |
