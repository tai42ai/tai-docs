...
Register an `Agent` subclass under ``name`` and auto-register its JSON ``run`` tool.
...
```python
async pending_messages(self, thread_id: str, *, after: str) -> list[PendingMessage]
```
...
`PendingMessage`. Read-only and in-process (it reads the same thread index the turn
...
`TurnSupersededError`) before an expensive or
...
```python
async assert_resume_authorized(self, interaction_id: str) -> None
```
...
Raises `ParkResumeUnauthorizedError` for an external
...
```python
assert_delivery_authorized(self, completion_id: str | None) -> None
```
...
`ParkDeliveryUnauthorizedError` otherwise, so a caller
...
```python
redelivery_horizon_seconds(self) -> int
```
...
The bound, `Visit`-typed shared-visit callable.
...
`VisitOutcome`. A door resolves its own jqs to plain
...
```python
park_answer(self, outcome: VisitOutcome) -> Any
```

The ONE park-answer shape a direct door hands back for a `VisitOutcome`.
...
```python
async normalise_started(self, value: Any) -> VisitOutcome
```

Classify a raw start return into a ``started`` `VisitOutcome` over the ambient subject.

The same normalisation `visit` applies to what its ``start`` returned — caller asks,
a re-park, or a final result — exposed for a door that already ran its start INSIDE its own
`visit` (a hook fire whose visit owns the run) and only needs the return classified,
...
```python
async list_parked(self) -> list[ParkedEntry]
```
...
de-duplicated, each a `ParkedEntry` with its id, status
...
```python
async list_parked_for(self, context: StateContext | None) -> list[ParkedEntry]
```

Every parked interaction on ``context``'s subject — the same entries as `list_parked`.

A parkable-driving door fetches this over its OWN
`StateContext` once and feeds the list to the door-contract
...
```python
current_fire_identity(self) -> tuple[str, str] | None
```
...
```python
bound_execution_identity_for_fire(self, execution_key: str, fingerprint: str) -> AbstractAsyncContextManager[Any]
```
...
```python
async resume_parked(self, interaction_id: str, payload: Any = ...) -> VisitOutcome
```
...
`ParkableRunFailedError`. A live inline receiver, so the
...
```python
async cancel_parked(self, ids: list[str]) -> VisitOutcome
```
...
`VisitOutcome` naming the ids cancelled. An id outside
the run's own parked list raises `ParkedEntryGoneError`
...
The bound, `Ask`-typed ``ask`` callable.
...
```python
async get_active_versioned_body(self, name: str) -> tuple[int, PresetBody]
```
...
`PresetNotFoundError` for an unknown preset.
...
```python
async used_by(self, name: str) -> list[str]
```
...
`PresetNotFoundError` for a name that is not a known
preset.
...
Register the `Sandbox` provider (decorator form, returning the class unchanged).
...
Never gate execution on this nullable read; acquire through `require_sandbox`, the every-door
...
Raise `SandboxUnavailableError` (naming ``TAI_MCP_SANDBOX`` and
...
Raises `DeclarationInUseError` when a consumer still binds it.
...
Raises `TemplateInUseError` while it is still attached.
...
`tai42_contract.app.facets.states.AppStates.apply_batch`

```python
async apply_batch(self, writes: list[StateBatchWrite]) -> list[ApplyResult]
```

Apply an ordered write set as ONE transaction, returning an `ApplyResult` per item in order.

Each `StateBatchWrite` is dispatched by its shape — an
``ops`` batch through `apply`, a ``template_jq`` program through
`apply_template_jq` (its outcome mapped onto the item's ``ApplyResult``) — over the
...
`tai42_contract.app.facets.states.AppStates.open_unit`

```python
open_unit(self) -> AbstractAsyncContextManager[StateUnit]
```

Open a unit of work over the states facet, bound to the caller's scope for the ``async with`` block.

Yields a `StateUnit` the caller stages writes against; while
...
discard is logged. Nested runs join the outermost unit through `StateUnit.savepoint`.
...
`SubjectFoldError`; a retried fold to the same target is a quiet no-op.
...
The ambient `StateContext` the current door deposited.
...
```python
mcp_sub_app_router: SubMcpAppRouter
```
...
```python
async declared_extras(self, name: str) -> frozenset[str]
```
...
Register a `StateTemplateDetachReferee` consulted before a state-template detach.
...
Declare ``base_tool``'s registration tier (a `RouteAction`).
...
The projection `AppConversations.pending_messages` returns, so a body running inside a
