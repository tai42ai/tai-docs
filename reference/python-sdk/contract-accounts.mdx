...
```python
async has_login(self, user_id: str) -> bool
```
...
```python
async attach_login(self, user_id: str, *, credential: LoginCredential) -> LoginAttachment
```
...
```python
iter_accounts_provider_factories_staged() -> list[tuple[str, Callable[..., AccountsProvider]]]
```
