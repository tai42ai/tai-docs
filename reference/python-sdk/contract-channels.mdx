...
A `Channel` pushes an ``ask`` question to a human on a specific
...
`tai42_contract.channels.options.NOTIFICATION_FOOTER_MAX_CHARS`
...
`AppChannels.handle_inbound_answer` WITHOUT releasing the
...
`tai42_contract.channels.options.check_footer`
...
`ChannelNotification` and `AnswerPart` share these,
