---
title: "LLM factories (tai42_kit.llm)"
description: "LLM, embedding, checkpoint, and store factories."
icon: "brain"
---


LLM provider factories + the LangGraph checkpoint/store registries.

The chat-model and embedding factories cache per ``(provider, kwargs)`` and
import their langchain provider package lazily inside the build; the registry
accessors return the running event loop's singletons that pool checkpoint/store
connection resources. Importing this package pulls ``langchain_core`` /
``langgraph`` — that is the opt-in boundary (the ``tai42_kit`` top level stays
vendor-free).

## checkpoint_registry

`tai42_kit.llm.checkpoint.checkpoint_registry.checkpoint_registry`

```python
checkpoint_registry() -> CheckpointRegistry
```

Return the CheckpointRegistry for the running event loop.

One registry per loop; after ``close_all()`` the next call builds a fresh
registry instead of returning the closed one.


## get_embedding

`tai42_kit.llm.embedding.get_embedding`

```python
get_embedding(provider: str, **kwargs) -> Embeddings
```

Build (or return the cached) ``Embeddings`` client for ``provider`` with the given kwargs.

**Parameters**

| Parameter | Type | Default | Description |
|---|---|---|---|
| `provider` | `str` | — | — |
| `kwargs` | — | `{}` | — |


## get_embedding_async

`tai42_kit.llm.embedding.get_embedding_async`

```python
async get_embedding_async(provider: str, **kwargs) -> Embeddings
```

Build (or return the cached) ``Embeddings`` client for ``provider``, off the event loop.

**Parameters**

| Parameter | Type | Default | Description |
|---|---|---|---|
| `provider` | `str` | — | — |
| `kwargs` | — | `{}` | — |


## get_llm

`tai42_kit.llm.models.get_llm`

```python
get_llm(provider: str, **kwargs) -> BaseChatModel
```

Build (or return the cached) chat model for ``provider`` with the given kwargs.

**Parameters**

| Parameter | Type | Default | Description |
|---|---|---|---|
| `provider` | `str` | — | — |
| `kwargs` | — | `{}` | — |


## get_llm_async

`tai42_kit.llm.models.get_llm_async`

```python
async get_llm_async(provider: str, **kwargs) -> BaseChatModel
```

Build the chat model for ``provider`` in a worker thread, off the event loop.

**Parameters**

| Parameter | Type | Default | Description |
|---|---|---|---|
| `provider` | `str` | — | — |
| `kwargs` | — | `{}` | — |


## store_registry

`tai42_kit.llm.store.store_registry.store_registry`

```python
store_registry() -> StoreRegistry
```

Return the StoreRegistry for the running event loop.

One registry per loop; after ``close_all()`` the next call builds a fresh
registry instead of returning the closed one.


## system_prompt_cache_mark

`tai42_kit.llm.models.system_prompt_cache_mark`

```python
system_prompt_cache_mark(provider: str) -> dict[str, Any] | None
```

The content-block kwargs that mark ``provider``'s system prompt for caching, or ``None``.

A provider that takes an explicit system-prompt cache breakpoint returns the
``system_content_kwargs`` that carry it (the block `build_system_message` merges onto
the system message); a provider that caches its prefixes with no mark, or does not cache,
returns ``None``. The ``match`` mirrors `_build_llm`'s so a provider name never leaves
this module and an unknown provider is refused loudly rather than silently un-marked.

**Parameters**

| Parameter | Type | Default | Description |
|---|---|---|---|
| `provider` | `str` | — | — |

