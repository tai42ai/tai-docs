...
| `--file` `&lt;str&gt;` | Store the raw bytes read from this file path, or from stdin when the path is '-', keeping secret content off the command line (a value on argv leaks via ps and shell history). |
| `--json` / `--no-json` | Emit raw JSON instead of human tables. |
