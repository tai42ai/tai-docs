...
| `--condition-kwargs-file` `&lt;str&gt;` | Read the condition kwargs JSON object from a file, or from stdin when the path is '-', instead of putting a secret on the command line (a value on argv leaks via ps and shell history). Mutually exclusive with --condition-kwargs. |
