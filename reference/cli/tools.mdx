...
| Option | Description |
| --- | --- |
| `--combo` `TEXT` | One extension combo as a JSON array of elements (repeatable). Each element is an extension name ('"chain"') or a &#123;"name","config"&#125; object binding config to it (e.g. '[&#123;"name":"output_schema","config":&#123;"schema":&#123;"type":"object"&#125;&#125;&#125;]'). Passing no --combo clears the tool's extensions. |
