---
title: "tai agents"
description: "List and run agents."
icon: "terminal"
---

List and run agents.

```console
$ tai agents [OPTIONS] COMMAND [ARGS]...
```

## `tai agents authored-run`

Stream a run of an authored agent (a preset baked over a spec-runnable agent).
Exactly one of `--input` or `--input-file` is required.

Example: `tai agents authored-run my_researcher --input '&#123;"query":"weather"&#125;'`

```console
$ tai agents authored-run [OPTIONS] {name}
```

| Argument | Description |
| --- | --- |
| `name` | Authored-agent (preset) name. _(required)_ |

| Option | Description |
| --- | --- |
| `--input` `&lt;str&gt;` | The non-baked agent input as a JSON object. |
| `--input-file` `&lt;str&gt;` | Read the agent input JSON object from a file, or from stdin when the path is '-', instead of putting a secret on the command line (a value on argv leaks via ps and shell history). Mutually exclusive with --input. |
| `--json` / `--no-json` | Emit raw JSON instead of human tables. |

## `tai agents list`

List every registered agent.

Example: `tai agents list`

```console
$ tai agents list [OPTIONS]
```

| Option | Description |
| --- | --- |
| `--json` / `--no-json` | Emit raw JSON instead of human tables. |

## `tai agents run`

Stream a run of an agent, one event frame at a time. Exactly one of `--input` or
`--input-file` is required.

Example: `tai agents run researcher --input '&#123;"query":"weather"&#125;'`

```console
$ tai agents run [OPTIONS] {name}
```

| Argument | Description |
| --- | --- |
| `name` | Agent registration name. _(required)_ |

| Option | Description |
| --- | --- |
| `--input` `&lt;str&gt;` | The agent input as a JSON object. |
| `--input-file` `&lt;str&gt;` | Read the agent input JSON object from a file, or from stdin when the path is '-', instead of putting a secret on the command line (a value on argv leaks via ps and shell history). Mutually exclusive with --input. |
| `--json` / `--no-json` | Emit raw JSON instead of human tables. |

## `tai agents spec-runnable`

List the spec-runnable (authorable) agents.

Example: `tai agents spec-runnable`

```console
$ tai agents spec-runnable [OPTIONS]
```

| Option | Description |
| --- | --- |
| `--json` / `--no-json` | Emit raw JSON instead of human tables. |
