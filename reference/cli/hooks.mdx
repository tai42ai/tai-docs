...
| Option | Description |
| --- | --- |
| `--execution-key` `&lt;str&gt;` | The api-key user id the link's dispatch is gated on; revoking it kills the link. _(required)_ |
| `--name` `&lt;str&gt;` | Link name (the revocation handle); a unique name is generated when omitted. |
| `--ttl` `&lt;int&gt;` | Link lifetime in seconds (a timed link). |
...
| `--params` `&lt;str&gt;` | Per-link tool_kwargs as a JSON object, filling only the arguments a hook leaves unpinned. |
| `--params-file` `&lt;str&gt;` | Read the params JSON object from a file, or from stdin when the path is '-', instead of putting a secret on the command line (a value on argv leaks via ps and shell history). Mutually exclusive with --params. |
...
| Option | Description |
| --- | --- |
| `--params` `&lt;str&gt;` | The hook register body as a JSON object. |
| `--params-file` `&lt;str&gt;` | Read the params JSON object from a file, or from stdin when the path is '-', instead of putting a secret on the command line (a value on argv leaks via ps and shell history). Mutually exclusive with --params. |
| `--json` / `--no-json` | Emit raw JSON instead of human tables. |
...
| Option | Description |
| --- | --- |
| `--verifier` `&lt;str&gt;` | Registered webhook-verifier name. _(required)_ |
| `--config` `&lt;str&gt;` | Verifier config as a JSON object. |
| `--json` / `--no-json` | Emit raw JSON instead of human tables. |
