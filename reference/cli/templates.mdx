---
title: "tai templates"
description: "Manage prompt and resource templates."
icon: "terminal"
---

Manage prompt and resource templates.

```console
$ tai templates [OPTIONS] COMMAND [ARGS]...
```

## `tai templates clear-cache`

Clear the template render cache.

Example: `tai templates clear-cache`

```console
$ tai templates clear-cache [OPTIONS]
```

| Option | Description |
| --- | --- |
| `--json` / `--no-json` | Emit raw JSON instead of human tables. |

## `tai templates delete`

Delete a template.

Example: `tai templates delete prompts/greeting.md`

```console
$ tai templates delete [OPTIONS] {path}
```

| Argument | Description |
| --- | --- |
| `path` | Template key to delete. _(required)_ |

| Option | Description |
| --- | --- |
| `--json` / `--no-json` | Emit raw JSON instead of human tables. |

## `tai templates delete-dir`

Delete every template under a directory.

A directory that matches no templates is a loud 404 (unlike `delete`, which
is idempotent for an absent single key).

Example: `tai templates delete-dir prompts/archive`

```console
$ tai templates delete-dir [OPTIONS] {path}
```

| Argument | Description |
| --- | --- |
| `path` | Template directory to delete. _(required)_ |

| Option | Description |
| --- | --- |
| `--json` / `--no-json` | Emit raw JSON instead of human tables. |

## `tai templates get`

Fetch a template's content and its input schema.

Example: `tai templates get prompts/greeting.md`

```console
$ tai templates get [OPTIONS] {template_id}
```

| Argument | Description |
| --- | --- |
| `template_id` | Template id. _(required)_ |

| Option | Description |
| --- | --- |
| `--json` / `--no-json` | Emit raw JSON instead of human tables. |

## `tai templates list`

List the available templates.

Example: `tai templates list`

```console
$ tai templates list [OPTIONS]
```

| Option | Description |
| --- | --- |
| `--json` / `--no-json` | Emit raw JSON instead of human tables. |

## `tai templates render`

Render a template by id or inline content with kwargs.

Example: `tai templates render --template-id prompts/greeting.md --kw name=Ada`

```console
$ tai templates render [OPTIONS]
```

| Option | Description |
| --- | --- |
| `--template-id` `&lt;str&gt;` | A stored template id to render. |
| `--content` `&lt;str&gt;` | Inline template content to render. |
| `--kwargs` `&lt;str&gt;` | Render kwargs as a JSON object. |
| `--kwargs-file` `&lt;str&gt;` | Read the render kwargs JSON object from a file, or from stdin when the path is '-', instead of putting a secret on the command line (a value on argv leaks via ps and shell history). Mutually exclusive with --kwargs; --kw pairs still override its keys. |
| `--kw` `&lt;str&gt;` | A key=value render kwarg (repeatable; value parsed as JSON). |
| `--json` / `--no-json` | Emit raw JSON instead of human tables. |

## `tai templates upload`

Upload (create or overwrite) a template from a local file.

Example: `tai templates upload prompts/greeting.md --file greeting.md`

```console
$ tai templates upload [OPTIONS] {path}
```

| Argument | Description |
| --- | --- |
| `path` | Template key to write. _(required)_ |

| Option | Description |
| --- | --- |
| `--file` `&lt;file&gt;` | Local file whose content to upload. _(required)_ |
| `--json` / `--no-json` | Emit raw JSON instead of human tables. |
