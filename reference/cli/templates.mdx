...
| Option | Description |
| --- | --- |
| `--text` `&lt;str&gt;` | The templated text to render as a JSON object: inline Jinja under "content" OR a stored resource id under "id" (exactly one), plus any render parameters under "kwargs". |
| `--text-file` `&lt;str&gt;` | Read the templated-text JSON object from a file, or from stdin when the path is '-', instead of putting a secret on the command line (a value on argv leaks via ps and shell history). Mutually exclusive with --text. |
| `--json` / `--no-json` | Emit raw JSON instead of human tables. |
