...
A preset a module ships (a `module:<name>` tag) is **generated machinery** its loop card
mints, not a user building block, so it lists **inspect-only**: a non-draggable card carrying a
**"generated"** badge that opens the same inspect view on click — **read-only**, with no
Edit/Delete controls. The canvas refuses to place one, and a flow whose module status is not
yet known stays read-only until it resolves.
