...
- **Start.** The `start_expr` jq that shapes the held call's input (a holder has no
  ...
  giving null starts nothing, and any other result merges over the resolved node input
  (a non-dict result wrapped as `{input: …}`), the expression's keys winning on
  collision. A Flow Call
