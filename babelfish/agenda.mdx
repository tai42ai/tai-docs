...
The mount's `declarations` carry `intents`, `phases`, `outcomes`, and `parties` — the wait
parties beyond the engine's own — plus `clarify_kind`, `unmatched_kind`, `clarify_limit`,
...
mount, as does a list on a `close`, which names one item.
...
| `template` | a rendered provider template | `text` (mandatory) plus `{id, language, params: {<param>: <field>}}` — `language` is the provider template's registered language code (not a locale the module selects) |
...
facts, the fixed parts, the prior transcript window, the inbound text, the prompt,
and the classifier's `turn_talk` text; the preset speaks ONE verified reply
...
| the clarify kind lacks an `unresolved` label carrying a declared not-done resolution | `clarify kind '<kind>' must declare an 'unresolved' label with a not-done resolution (a resolution other than 'done', from the mount parameter \`resolutions\`)` |
...
| no phase of the unmatched kind carries a not-done label | `the unmatched kind '<kind>' must declare a phase with a not-done label (a resolution other than 'done', from the mount parameter \`resolutions\`)` |
...
manifest declares — the agenda's `standing` (`result: "boolean"`) and `via`
(`result: "string"`). Each body's INPUT is the flow **root** and the mount
...
| a body's value type contradicts its declared `result` (a `standing` yielding a non-boolean, a `via` yielding a non-string) | `module 'agenda' compiled view '<view>' at <where> evaluated to <value> over the sample flow root, but the view declares result '<result>' — the body must yield a <result>` |
...
### Re-mounting over open intents

A mount that replaces the manifest while intents are still open is checked against those open
records too. When an open intent names a kind, a phase, an outcome label, or a part the NEW
manifest no longer declares, the mount is **REFUSED** with `422` and the refusal lists every
orphaned record — the declarations never change under a conversation whose next turn could no
longer resolve:

```
re-mounting module '<module>' on state '<state>' would orphan <N> open record item(s) the new
declarations no longer cover: [<kind>:<key>] <label> (id <id>); … and <N> more. Re-mount with
options {"orphans": "close", "resolution": "<not-done resolution>"} to close them.
```

Open intents whose declarations survive are untouched: their ledger rows, waits, and bindings
cross the mount unchanged.

To retire those declarations anyway, the caller mounts with `options.orphans = "close"` and
...
refused). The mount then **closes** every intent the new manifest no longer covers with that
...
- **`resolutions`** — the enum schema substituted into the `resolution` field at the
  ...
  the phase's successors. The presets read the allowed set at run time as
...
declaration-[compiled views](/babelfish/index#compiled-views) `agenda_standing` and
`agenda_via` are the exception: their input is the flow **root**, with the envelope
...
| `longest_chain` | `()` | the static pass bound, counted PER RECORD: the longest path in NODES over the `(kind, phase)` DAG of `predecessor` and ordering-wait edges — the MAX over the graph's weakly-connected components (parallel intents fire the same pass), a sequence group of N declared kinds multiplying its members' chain by N (they run serially). A `follow_on.mint` edge does not extend a path: the minted record starts a FRESH count, so re-entering a kind through a mint edge is legitimate and adds nothing to the bound |
...
- **`agenda-outcome`** — the loop's `outcome` role, generated after every handler branch. It
  ...
  mounted `resolutions`; a `wait_for` label's derived party is `user`, `phase`, or one of the
  manifest's declared `parties`; a `who: "phase"` label requires the handler's `waits_on`), and
...
- **`agenda-talk`** — the composed-talk step, invoked by `agenda-compose` when the module
  ...
  the `prompt_ref` template id through `render_text`, runs `tools_agent` constrained to the
...
- **`text`** renders through `render_text` over the part's content-store template id, the
  ...
  subject — the module selects no language variant. A `text` that is not a template id string
...
every fully-settled intent, the `sent` run stamp on each rendered row (the guard that sends
a wait's question exactly once, never re-asking an open wait whose question already went out),
`last_outbound` `{run}` whenever this turn speaks (a silent
...
- **A hit cap is a bug.** `max_iterations` (default **20**) is the backstop, not a
  ...
  nodes over one record's `(kind, phase)` DAG of predecessor and ordering-wait edges, a
  ...
  through a mint edge is legitimate. The Studio refuses a resolved cap below the bound before
  the save, and when the manifest declares its own `max_passes`, the mount check refuses a
  `longest_chain` above it.
...
the record step's `sources` gate, so the check enforces the unmatched kind's admission statically (its `sources`
