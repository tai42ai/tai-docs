...
The **`update`** template jq write the ledger. Each reads the current subtree as `.` and the
value the [binding adapter](/babelfish#binding-a-node-to-a-state) shaped from the node's output
as `$input` — and returns a template-relative op batch under the writer
