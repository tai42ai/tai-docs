...
- **The leaf rule.** Among `tai-*` packages, the skeleton imports `tai42-contract` and `tai42-kit` only; a plugin imports `tai42-contract` (and `tai42-kit` where it needs shared helpers) only. No plugin imports the skeleton — providers register through the `tai42_app` handle when the manifest loads them.
