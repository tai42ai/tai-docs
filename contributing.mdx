...
<CardGroup cols={2}>
  <Card title="tai-contract" icon="file-contract" href="https://github.com/tai42ai/tai-contract">
    The pure interface — the Protocols and ABCs plugins implement.
  </Card>
  <Card title="tai-kit" icon="toolbox" href="https://github.com/tai42ai/tai-kit">
    The generic leaf helpers, settings, pooled clients, and factories.
  </Card>
  <Card title="tai-skeleton" icon="server" href="https://github.com/tai42ai/tai-skeleton">
    The framework body — the concrete server and runtime engines.
  </Card>
  <Card title="tai-toolbox" icon="wrench" href="https://github.com/tai42ai/tai-toolbox">
...
- **The leaf rule.** Among `tai-*` packages, the skeleton imports `tai-contract` and `tai-kit` only; a plugin imports `tai-contract` (and `tai-kit` where it needs shared helpers) only. No plugin imports the skeleton — providers register through the `tai_app` handle when the manifest loads them.
...
Each repo's dev venv resolves its `tai-*` dependencies from sibling checkouts. The standard loop is the same across repos:
