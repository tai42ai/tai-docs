...
[connector](/concepts/connectors): one OAuth connection per user that unlocks
...
describes. Each sub-service is launched by the engine as a stdio MCP server, so
nothing extra runs on your side.
