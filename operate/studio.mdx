...
With access control on, a refresh or bookmark of an inner Studio route — `/agents`,
`/settings`, and the rest — serves the dataless SPA shell, not a `404`, so **deep
links work without pinning `/` or any SPA path public**: you never add the app
routes or asset prefixes to the public-route table. The
[Studio SPA shell](/concepts/access-control#the-studio-spa-shell-is-public-its-data-is-not)
concept covers why the shell leaks nothing and what stays behind `/api/*`.
