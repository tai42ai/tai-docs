...
The bucket policy needs `s3:GetObject`, `s3:PutObject`, `s3:DeleteObject`,
`s3:DeleteObjects`, and `s3:ListBucket` on the bucket and its contents; those are
the only calls the driver makes.
