...
A one-shot `db-init` step runs `tai db apply` before either long-lived process
starts. The DDL is additive and `IF NOT EXISTS`, so re-running it is safe.
...
`config/` is bind-mounted at `/app` in every app service, so `config/manifest.yml`
...
The repo ships a real Helm chart at `charts/tai`, published as an OCI artifact:
...
The chart integrates with cluster add-ons but never installs them: an Ingress
controller, cert-manager for `ingress.tls`, the prometheus-operator for
`metrics.serviceMonitor`, and a StorageClass for persistence.
...
[connectors](/concepts/connectors) resolve their OAuth redirect against it, and
`tai42-accounts-oidc` requires an https `PUBLIC_BASE_URL`.
