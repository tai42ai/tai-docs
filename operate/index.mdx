...
plugin is live with no restart. See [Install a plugin](/marketplace/install). For
...
the shared `ReadWriteMany` mount is required. A restarted or freshly scaled pod
picks the shared prefix up at boot; live pickup by ALREADY-RUNNING peer pods —
one pod installs, the others see it with no restart — is future work. Fleets
that want fully static rosters can bake them into a
