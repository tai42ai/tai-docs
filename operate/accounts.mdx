...
<Note>
Neither package is on PyPI yet, so install both from their repositories.
</Note>
...
```bash
pip install git+https://github.com/tai42ai/tai-accounts-postgres
```
...
Sessions and login backoff live in Redis, which arrives from the access-control
...
race, so read it from that process's logs. Either way the token is required:
there is no path that opens the route without one.
...
```bash
pip install git+https://github.com/tai42ai/tai-accounts-oidc
```
...
Each provider row takes a `name` (lowercase slug, unique), a `preset`, the client
credentials, and optionally `scopes`, `claim`, and a `display` label and icon.
The presets are `google`, `auth0`, `okta`, `keycloak`, `azure`, and `github`;
`google` and `github` carry fixed endpoints, and the others need a per-tenant
`issuer`.
