...
The provider carries no connection of its own. It binds to a [named
database](/concepts/config-and-secrets#named-databases) through
`TAI_DB_BINDING_TAI42_ACCOUNTS_POSTGRES` (default `default`) and uses that
database's connection, keeping its own tables inside it. A single-database
deployment leaves the binding unset and configures `TAI_DATABASE_DEFAULT_PG_*`;
give accounts its own database by binding it to another declared name and
configuring that one:

```bash
# accounts on the default database — nothing to set beyond the database itself
...
The provider is on when its bound database is configured — a non-empty password —
and cleanly OFF otherwise, like every other database-backed feature. There is no
baked-in default; the `localhost` values above are a local-dev recipe. Pool sizes
and timeouts are tuned on the database itself (`TAI_DATABASE_<NAME>_PG_*`), not a
variable of this package's own.
