...
`TAI_ACCOUNTS_PG_*` fields. There is no baked-in default for the connection
identity: the host and password resolve from this namespace, then from the shared
`TAI_DEFAULT_PG_*`, then absent — supply them here (the `localhost` values below are
a local-dev recipe, not a default) or through `TAI_DEFAULT_PG_HOST` /
`TAI_DEFAULT_PG_PASSWORD`.

```bash
TAI_ACCOUNTS_PG_HOST=localhost
TAI_ACCOUNTS_PG_PORT=5432
TAI_ACCOUNTS_PG_DB=tai
TAI_ACCOUNTS_PG_USER=postgres
TAI_ACCOUNTS_PG_PASSWORD=...
```

| Variable | Default | Effect |
| --- | --- | --- |
| `TAI_ACCOUNTS_PG_HOST` | — | Postgres host. No baked-in default; resolves from `TAI_DEFAULT_PG_HOST` when unset. |
| `TAI_ACCOUNTS_PG_PORT` | `5432` | Postgres port. |
| `TAI_ACCOUNTS_PG_DB` | `tai` | Database name. |
| `TAI_ACCOUNTS_PG_USER` | `postgres` | Role the provider connects as. |
| `TAI_ACCOUNTS_PG_PASSWORD` | — | Password for that role. No baked-in default; resolves from `TAI_DEFAULT_PG_PASSWORD` when unset. |
| `TAI_ACCOUNTS_PG_MIN_CONNECTIONS` | `2` | Pool floor. |
| `TAI_ACCOUNTS_PG_MAX_CONNECTIONS` | `10` | Pool ceiling. |
| `TAI_ACCOUNTS_PG_CONNECT_TIMEOUT` | `10` | Connect timeout in seconds. |
| `TAI_ACCOUNTS_PG_STATEMENT_TIMEOUT_SECONDS` | `60` | Per-statement timeout in seconds. |
...
Three tables — users, sessions, and invites — are created by the plugin's own
migration chain. The plugin ships a `migrations/` folder and declares it in its
descriptor, so the shared runner discovers it as the `tai42-accounts-postgres`
component and applies it alongside the core chain:

```bash
tai db migrate
```

`tai db status` shows every component's applied / pending verdict, including this
one. The provider asserts its chain at boot and never applies it for you: with the
accounts store configured but the chain pending, the process refuses to start,
naming `tai db migrate`. Run the migrations, then start the server.
