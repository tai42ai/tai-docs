...
Decorate a factory with `@tai_app.extensions.extension`, naming the kind and the extension name. The factory returns a new callable named `<tool>_<extension>`.
...
- [`tai-toolbox`](https://github.com/tai42ai/tai-toolbox) — `cache`, `proxy`, and `prometheus_metrics` (wrappers); `batch`, `chain`, and `output_schema` (transformers).
...
- [Python SDK reference](/reference/python-sdk) — `ExtensionKind` and the `tai_app.extensions` surface.
