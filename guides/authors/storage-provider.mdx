...
Decorate the class with `@tai_app.storage.register_storage`, as the example does. The registry is dead by default — it holds no provider until one is registered, and the registered class is instantiated as the active provider. One provider per process.
