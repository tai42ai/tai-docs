...
Depend on `tai-contract` (and `tai-kit` for shared LLM factories) only; never import the skeleton. The host loads the agents by naming the module under `agents[]`, and drives them over SSE by loading the agents router:
