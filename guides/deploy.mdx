...
**Pin the probe routes public.** With access control enabled, unknown routes are denied, so `/health` and `/ready` must be public for probes that carry no key. Unlike the Studio shell, these are registered operational routes — the SPA-shell fallback never covers them, so pin each one explicitly through the public-routes door (the `public` marker); there is no `"*"` wildcard. A distribution compose that carries them in its `ACCESS_CONTROL_ALWAYS_PUBLIC_PATH_PREFIXES` needs no further step.
