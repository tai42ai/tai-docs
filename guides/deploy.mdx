...
**Public SPA prefixes when serving Studio.** With access control on, unknown routes are denied — including the Studio SPA shell and its static assets, which are not route-table rows. So a Studio-serving deployment must make the SPA's static surface public, or its login page never loads. `ACCESS_CONTROL_ALWAYS_PUBLIC_PATH_PREFIXES` **replaces** the code default (`/api/login`) wholesale, so re-include `/api/login` and the operational probes (`/health`, `/ready` — see [Liveness and readiness](#liveness-and-readiness) below) alongside the SPA prefixes:

```bash
export ACCESS_CONTROL_ALWAYS_PUBLIC_PATH_PREFIXES='["/api/login", "/assets", "/vendor", "/favicon.ico", "/tai42-logo-icon.png", "/apple-touch-icon.png", "/", "/health", "/ready"]'
```

Everything under `/api/*` except `/api/login` stays authed. The distribution compose ships this default; a headless (no-Studio) deployment can drop the SPA prefixes, but keep `/api/login`, `/health`, and `/ready`.
...
**Pin the probe routes public.** With access control enabled, unknown routes are denied, so `/health` and `/ready` must be public for probes that carry no key. The distribution compose's `ACCESS_CONTROL_ALWAYS_PUBLIC_PATH_PREFIXES` default above already includes both, so a compose deployment needs no further step. Otherwise, pin them at runtime through the public-routes door (the `public` marker); there is no `"*"` wildcard.
