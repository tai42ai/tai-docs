...
```bash
tai sub-mcp register utilities --tool convert --tool lookup
```
...
<Steps>
  ...
  <Step title="Point a client at the slug">
    An MCP client connects to the sub-MCP endpoint the same way it connects to the main server, at the slug's own mount: `/app/{slug}` — so the `utilities` app above answers at `https://<your-server>/app/utilities`. A `tai sub-mcp register` app is served over the streamable **HTTP** transport (the default), the same transport the main MCP endpoint uses. See [MCP clients](/integrations/mcp-clients).
  </Step>
  <Step title="Unregister an app">
    ```bash
    tai sub-mcp delete utilities
    ```
  </Step>
</Steps>
