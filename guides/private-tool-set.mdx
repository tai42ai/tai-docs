...
<Steps>
  ...
  <Step title="Point a client at the slug">
    An MCP client connects to the sub-MCP endpoint the same way it connects to the main server, at the slug's own mount: `/app/{slug}` — so the `billing` app above answers at `https://<your-server>/app/billing`. A `tai sub-mcp register` app is served over the streamable **HTTP** transport (the default), the same transport the main MCP endpoint uses. See [MCP clients](/integrations/mcp-clients).
  </Step>
