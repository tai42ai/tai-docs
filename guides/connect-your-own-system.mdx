...
<Steps>
  <Step title="Mint the invoking key">
    ...
    ```bash
    tai keys create --user account-app --description 'invokes the account route' --scope conversations
    ```
  </Step>
  <Step title="Mint the execution key">
    ...
    ```bash
    tai keys create --user account-agent --description 'account route agent' --scope read
    ```
  </Step>
</Steps>
...
An `api` route may carry an HTTPS `callback_url` — the sink the answer is POSTed to;
this walkthrough uses one. When set, the URL **must** be absolute HTTPS; an `http://`
or relative URL is rejected at create. A route with no `callback_url` reads its answers
back from the poll door instead (below).

```bash
tai conversations create account \
  --door api --target-name account-agent-spec --execution-key account-agent \
...
```json
{
  "created": true,
  "route_name": "account",
  "route": { "door": "api", "target_kind": "agent", "target_name": "account-agent-spec", "callback_url": "https://your-app.example.com/tai/answers" },
...
```bash
curl -sS -X POST "$TAI_BASE_URL/api/conversations/account/messages" \
  -H "X-Api-Key: $INVOKING_KEY" \
  -H 'Content-Type: application/json' \
  -d '{"external_user_id": "user-42", "text": "What is my usage?"}'
```

The default answer is `202` with the id to correlate the callback against:

```json
{ "data": { "message_id": "4f1c…", "thread_id": "bridge:account:user-42" } }
```

Without a `callback_url` on the route, read the answer back once the turn finishes from
`GET /api/conversations/{route_name}/messages/{message_id}`, keyed by the `message_id`
this `202` returned.
...
curl -sS -X POST "$TAI_BASE_URL/api/conversations/account/messages" \
  -H "X-Api-Key: $INVOKING_KEY" \
  -H 'Content-Type: application/json' \
  -d '{"external_user_id": "user-42", "text": "What is my usage?", "wait_seconds": 20}'
```

```json
{ "data": { "message_id": "4f1c…", "thread_id": "bridge:account:user-42",
            "answer": { "message_id": "4f1c…", "thread_id": "bridge:account:user-42",
                        "status": "answered", "answer": "Your usage is 12 runs this week." } } }
```
...
```json
{ "message_id": "4f1c…", "thread_id": "bridge:account:user-42",
  "status": "answered", "answer": "Your usage is 12 runs this week." }
```
...
On a **silent** turn — a `tool` target whose mapped reply was `null` or blank, or an
ordinary inbound on a [`manual`-mode
thread](/reference/conversation-bridge#control-mode) whose suppressed target turn
produced no reply — the callback carries `status: "silent"` and **no** `answer` key at
all:

```json
{ "message_id": "4f1c…", "thread_id": "bridge:account:user-42", "status": "silent" }
```
