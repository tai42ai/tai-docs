...
A connector provider registers when the manifest imports its module. Add the provider's module to a `lifecycle_modules` entry (importing it calls `tai_app.connectors.register_connector`).

```yaml manifest.yml
lifecycle_modules:
  - tai_connector.google.core.connector
```
