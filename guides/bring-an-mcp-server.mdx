...
<Steps>
  <Step title="Check binding status">
    ...
    The payload carries three blocks: `bound` maps each attached server's title to the tool names it serves, `failed` lists the servers a viability probe skipped — each entry carrying the server `title`, a coarse `status`, a `category` (`auth`, `unreachable`, or `error`), a redacted `message`, and the `http_status` the failure carried (`null` for a pure transport failure) — and `health` reports each server's most recent tool-dispatch outcome, keyed by title (a skipped server carries the block at its empty values).
