...
With the gate on and no key yet, there is no authenticated door to create one — `tai keys create` itself needs a key. The one-shot [`tai keys bootstrap`](/reference/cli/keys) door (`POST /api/keys/bootstrap`) closes that gap: it mints the first condition-free `["*"]` admin key, gated by a boot-time token.
