---
title: "Set up access control"
description: "Turn on the gate and write a policy."
icon: "shield-halved"
---

import VerifyAllowed from '/snippets/examples/access_control/verify_allowed.mdx';
import VerifyDenied from '/snippets/examples/access_control/verify_denied.mdx';
import IdentityManifest from '/snippets/examples/access_control/identity_manifest.mdx';
import KeysCreateHelp from '/snippets/examples/cli/keys_create_help.mdx';

Turn on the request-auth gate and write a jq access-control policy.
[Access control](/concepts/access-control) explains the model this configures.

The runtime gates its HTTP routes behind API keys and per-user policies. A request presents an `sk-…` key; the access-control middleware resolves the key's user and evaluates that user's jq policy against the request context. You provision keys, map routes to scopes, and attach jq conditions.

## Prerequisites

The gate is on by default. Before it can admit requests in a real deployment, these backing pieces must be in place.

<Steps>
  <Step title="Access control is enabled">
    The gate runs unless you set `ACCESS_CONTROL_ENABLE=false`. Leave it unset (or `true`) in production — the [quickstart](/getting-started/quickstart) disables it only for local, auth-free development.
  </Step>
  <Step title="A Redis for the live auth context">
    The identity provider's api-key records and the per-user live counters live in a plain Redis — any standard `redis-server`. No RedisJSON module or `redis-stack` image is required. There is no baked-in default: point the connection with `ACCESS_CONTROL_REDIS_URL`, or let it resolve from the shared `TAI_DEFAULT_REDIS_URL` — with the gate on, one must be set.
  </Step>
  <Step title="A Postgres policy store">
    Scopes, route mappings, and per-user policy bodies live in Postgres — the only policy store. It lives in the [named database](/concepts/config-and-secrets#named-databases) the core `skeleton` component binds to — the `default` database unless `TAI_DB_BINDING_SKELETON` points it elsewhere. Configure that database, including its password via `TAI_DATABASE_DEFAULT_PG_PASSWORD`.
  </Step>
  <Step title="An identity provider in the manifest">
    The skeleton ships no concrete identity provider, so a deployment with the gate on **must** name one as a lifecycle module in the manifest. The default is the redis-backed api-key provider:

    <IdentityManifest />

    It registers itself as the `redis` provider at import; `ACCESS_CONTROL_AUTH_PROVIDERS` (an ordered JSON list, default `["redis"]`) names the chain the verifier tries. Swap the module and the provider name to use a different authn backend.
  </Step>
  <Step title="Probe routes are public by default">
    The guard denies any route it has no mapping for **to ordinary identities** (a condition-free `*` super-admin key is admitted — a root identity is never gated by a missing route row). The liveness and readiness probes carry no key, but they are served public by the app's own route-level declaration: `/health` and `/ready` ship in `ACCESS_CONTROL_ACKNOWLEDGED_PUBLIC_ROUTES`, so an orchestrator reaches them with no key and no manual pin.
  </Step>
</Steps>

## Initialize the deployment

With the gate on and no principal yet, there is no authenticated door to mint a key — `tai keys create` itself needs a key. The one-shot [`tai setup`](/reference/cli/setup) door (`POST /api/setup`) closes that gap: it creates the **owner** principal — the first admin — and mints the owner's first condition-free `["*"]` key, returned once.

```bash
tai setup --display-name alice --token -
```

The door resolves its token in one of two ways:

- **`TAI_SETUP_TOKEN`** — an operator-set token. When set, it is the token the door expects.
- **the auto-generated token** — absent an operator token, each deployment fixes one random token once at startup and logs it a single time (`setup token: …`). Only someone who can read the server log can initialize the deployment.

Pass the token out of band, never on the command line — `--token -` reads it as one line from stdin (a value on `argv` leaks via `ps` and shell history).

The door is **one-shot and secure by default**: it refuses with `409 Already initialized` the moment any principal exists, a wrong or absent token is a generic `403` (with a per-IP backoff that escalates against a token flood), and with the gate off (`ACCESS_CONTROL_ENABLE=false`) it is disabled with a `501` — there is no deployment to initialize. For local, auth-free development, `TAI_SETUP_OPEN=true` opens the door without a token.

Once the owner key is in hand, every further key is created through the authenticated surface below.

### Initializing a deployment whose logins are OIDC

When your humans sign in through an external OIDC provider, that provider answers only **who** a caller is — it provisions nothing. An issuer-authenticated subject with no principal resolves to the empty policy, so every protected route denies it, and an OIDC identity provider mints no API keys. A fresh deployment still needs its first principal — the owner — and the setup door is the only way to create one. So the setup token matters even when every human logs in through OIDC: it guards the one door that mints the owner and the owner's first key.

Configure a **key-minting** identity provider beside the OIDC one in `ACCESS_CONTROL_AUTH_PROVIDERS` — the bundled `redis` API-key provider mints the owner's first key. The auto-generated setup token is minted and logged once, at `INFO`, only when the door can serve. No token is logged under four conditions; under the first the door still works — the token is your own value — and under the other three the door itself is unavailable and answers `501`:

- **`TAI_SETUP_TOKEN` is set** — that value is the token, and the door uses it directly.
- **no configured identity provider can mint API keys** — an OIDC-only chain mints none; add a key-minting provider.
- **the access-control Redis is unset** — the door needs it for the setup token, the throttle, and the mint lock.
- **access control is off** — there is no deployment to initialize.

The token is fixed once per deployment, so a restart finds it in place and never re-logs it. Initialize an OIDC deployment in this order:

<Steps>
  <Step title="Read the setup token">
    Take it from the server log, or pin it yourself with `TAI_SETUP_TOKEN`.
  </Step>
  <Step title="Create the owner and first key">
    Run [`tai setup`](/reference/cli/setup) — it mints the owner principal and the owner's first `["*"]` key, returned once.
  </Step>
  <Step title="Sign in with the key">
    The owner authenticates with that key while no interactive login exists yet.
  </Step>
  <Step title="Provision the OIDC subjects">
    The owner creates a principal for each OIDC subject through the admin principals door (`POST /api/auth/principals`) and grants it a role.
  </Step>
  <Step title="OIDC logins work">
    Each provisioned subject now signs in through OIDC; the setup door has closed for good.
  </Step>
</Steps>

On Kubernetes, pin the token ahead of boot: set `TAI_SETUP_TOKEN` from a Secret so the value is known before the pods start, rather than scraped from a replica's log.

## Provision an API key

Create a key for a user. The raw `sk-…` value is printed once — capture it then. Grant scopes with the repeatable `--scope`, and attach an inline jq authorization condition with `--condition`.

```bash
tai keys create --user alice --description 'CI key' --scope read
```

The scopes are granted with the repeatable `--scope` flag — the command's own help confirms it:

<KeysCreateHelp />

The CLI itself authenticates with a key. Pass it out of band — read one line from stdin with the global `--api-key-stdin` flag, or set `TAI_API_KEY` in the environment. There is no `--api-key VALUE` flag, because a value would leak via `ps` and shell history.

### Keys for automation

A machine identity is a **service principal** — a non-human principal that holds keys but never logs in. An admin creates one, then mints keys owned by it with `--for`:

```bash
tai principals create --kind service --display-name 'CI runner' --role editor --user ci-runner
tai keys create --user ci --description 'CI key' --scope read --for ci-runner
```

`--for` names the principal a key belongs to; omitted, a key is owned by the caller's own principal. Manage service principals with `tai principals list|disable|enable|delete`.

## Map routes to scopes

A scope maps URLs to a named policy target. Add a URL to a scope, optionally with a dynamic match pattern, and list the current mappings.

```bash
tai scopes add read /api/tools
tai scopes list
```

<Warning>
**Scope a pattern to a surface; never blanket the whole site.** Every API route already declares its own protection — a scope row or pattern *adds* scoping, it is not what keeps `/api` private. Deny wins: a path that matches both a protected pattern and a public mapping resolves protected, so a whole-site pattern like `^/.*$` also matches browser page navigations and blocks the dataless Studio shell — a deep-link refresh then meets a `401` JSON body instead of the app shell and its login flow, and the shell exposes no data anyway. Tighten a real surface with a scoped pattern (`^/api/tools/.*`), never a blanket. The plugin studio-asset door stays public through the `always_public_route_patterns` setting, which resolves it public **alone**, ahead of any route row (a plugin bundle loads as a native ESM import that cannot send an auth header); protect it by editing that setting, not with a scope row.
</Warning>

## Write and validate a jq condition

A jq condition compiles against the request context and returns whether the request is allowed. Validate one before you save it — compile it, and optionally sample-evaluate it against a `JqAuthContext`-shaped sample.

```bash
tai keys validate-condition --condition '.method == "GET"'
```

Attach the validated condition when you create or edit a key with `--condition`.

<Steps>
  <Step title="Draft the condition">
    Write a jq expression over the request context and compile-check it with `tai keys validate-condition`.
  </Step>
  <Step title="Attach it to a key">
    Create the key with `--condition` — a templated-text JSON object carrying either inline jq (`content`) or a stored condition (`id`), plus optional render `kwargs`.
  </Step>
  <Step title="Track policy changes">
    Each user's enforced policy is append-only and versioned. List the history with `tai keys policy-versions`, and roll back with `tai keys policy-rollback`.
  </Step>
</Steps>

<Warning>
A key is revoked immediately with `tai keys delete` — the next request with it fails to authenticate. Rotate a key by revoking and re-issuing; editing a key never rotates its material.
</Warning>

## Verify enforcement

Prove the gate before you trust it: call a guarded route with two keys. A key whose policy grants the route's scope reaches the resource with `200`; a valid key that lacks the scope is denied with a fixed `403`. Set `TAI_BASE_URL` to your server and the `X-Api-Key` header to each key.

An allowed key reaches the resource:

<VerifyAllowed />

A valid but under-scoped key is denied — a fixed `403`, with the full reason logged only server-side:

<VerifyDenied />

Both requests run against the real `AuthAdapter → AccessControlAuthBackend → ResourceGuardMiddleware` chain in docs CI, so a change that breaks allow/deny enforcement breaks this step.

## Delegate with owned keys

A key minted by a non-admin is an [owned key](/concepts/owned-keys) — capped at a
subset of its owner's scopes, isolated to its own data, and attenuated with the
owner on every request. To mint a capped key for a teammate or device, inspect what
it can do with `tai auth whoami`, and share it by one-time QR claim link, follow the
[owned-keys guide](/guides/owned-keys).

## See also

- [Access control](/concepts/access-control) — the key, scope, and jq policy model.
- [Owned keys](/concepts/owned-keys) — capped sub-identities, the capability
  projection, and data isolation.
- [The versioning spine](/concepts/versioning-spine) — the append-only policy history.
- [CLI reference](/reference/cli) — the full `tai keys` and `tai scopes` surface.
