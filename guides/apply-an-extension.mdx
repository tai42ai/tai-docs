...
To change one combo without rewriting the whole list, `tai tools extensions-add` appends a single combo and `tai tools extensions-remove` drops one, leaving the tool's other combos in place — adding a combo already present is refused, as is removing one the tool does not carry.

```bash
tai tools apply greet --combo '["cache"]'
tai tools extensions-add greet --combo '["batch"]'
tai tools extensions-remove greet --combo '["batch"]'
