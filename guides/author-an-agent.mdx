...
class EchoAgent(Agent):
    ...
    async def run(self, *, user_message: TemplatedText | None = None, **kwargs) -> str:
        if user_message is None:
            return ""
        return await render_templated_text(user_message)
```

A message reaches `run` as a `TemplatedText` — inline `content` or the `id` of a stored resource, plus its render `kwargs` — so `run` renders it to a string through the kit's `render_templated_text` seam before using it.
