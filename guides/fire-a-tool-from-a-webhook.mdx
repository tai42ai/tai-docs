...
door](/reference/conversation-bridge#the-event-door), not a hook: it enters that thread
...
1. the hook's `start_expr` output over the scan payload (weakest — the scan influences it),
2. the link's `--params`,
3. the hook's static `tool_kwargs` (strongest — a pin the link cannot override).

A scan's query string reaches the tool **only** when the hook opts in with a `start_expr` that maps it. Register a hook whose `start_expr` lifts `?name=` into the tool input:

```bash
tai hooks register --params '{"name":"greet","topic":"events","tool":"notify","start_expr":{"content":"{who: .name}"}}'
```

Now `GET /trigger/<token>?name=Ada` fires `notify` with `{"who":"Ada"}` — unless the link's `--params` or the hook's static `tool_kwargs` also set `who`, which win over the scanned value.
