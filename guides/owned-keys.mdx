...
holds itself; an admin may also mint keys for a service principal. Point `TAI_SERVER_URL`
...
<Info>
Ownership is exactly one level deep: an owned key can never itself mint keys. An
admin may also mint a key for a service principal. See
