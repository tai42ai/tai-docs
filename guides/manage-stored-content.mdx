...
```bash
tai templates render --text '{"id": "prompts/greeting.md", "kwargs": {"name": "Ada"}}'
```
...
The clear fans out **fleet-wide** over the [worker bus](/concepts/worker-bus) — it
evicts the compiled-template cache on every worker and returns a per-worker
