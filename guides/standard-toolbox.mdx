...
| Name | Kind | Summary |
|---|---|---|
| `confirm_stripe_payment` | Tool | Answer a payment ask from a projected checkout.session.completed event. |
| `create_stripe_checkout` | Tool | Create a Stripe Checkout Session and return its hosted payment URL. |
...
| `reconcile_stripe_payments` | Tool | Re-answer every paid Checkout Session in the lookback window the webhook path may have missed. |
