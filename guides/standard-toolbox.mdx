...
<Note>
The `request` tool fetches URLs server-side, so it passes tai's URL guard —
private, loopback, and link-local addresses are refused by default. On a
self-hosted deployment, reaching a service on the Compose network is an explicit
opt-in: see [Reaching a service on the Compose network](/self-hosted#reaching-a-service-on-the-compose-network).
</Note>

## The standard set

The tools and extensions the toolbox ships. This table is generated from the plugin registry, so it never drifts from the package.
...
| `output_schema` | Extension | Branch a tool into a variant that forces a configured JSON Schema as its output schema and validates every result against it. |
...
<Note>
The full, always-current list of toolbox tools and extensions — names, kinds, and each item's description — lives on the [toolbox plugin page](/plugins/tai42/toolbox). Per-tool specifics (formats, session behavior, the extras to install) live in the toolbox package's own documentation, linked from that page.
</Note>
...
- [Toolbox plugin](/plugins/tai42/toolbox) — the full toolbox and its repository.
