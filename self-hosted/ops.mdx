...
was applied under. Both commands connect through each component's bound [named
database](/concepts/config-and-secrets#named-databases) and its admin identity —
`TAI_DATABASE_<NAME>_PG_ADMIN_*`, the DDL-privileged role, falling back to that
database's runtime role when unset — distinct from the app's runtime store role.
