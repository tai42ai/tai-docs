...
```bash
helm install tai oci://ghcr.io/tai42ai/charts/tai --version 1.6.33
helm test tai
```
...
Redis StatefulSets for a self-contained trial. A `schemaInit` hook Job runs
`tai db migrate` around install and upgrade, applying the ordered migration chain
before the app pods start.
...
```bash
helm upgrade tai oci://ghcr.io/tai42ai/charts/tai --version 1.6.33
```

The upgrade re-runs the `tai db migrate` hook Job and rolls the Deployments. With
an external Postgres the hook runs `pre-upgrade` (migrations first, strict
ordering); with the quickstart Postgres it runs `post-upgrade`. The chain rolls
forward and each new pod refuses to serve until its schema is applied, so the
hook must complete before the rollout; the data lives in your datastores, not the
pods.
