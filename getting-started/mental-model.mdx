...
flowchart TD
    ...
    P["plugins<br/>connectors · storage · config · backends · monitoring · channels"] -->|register through tai42_app| R
```

## The layers

Four packages build on `tai42-contract`, each depending only on packages nearer
that leaf: `tai42-contract` declares the interfaces, `tai42-kit` supplies generic
helpers, `tai42-cli` provides the `tai` command, and `tai42-skeleton` is the
runnable server that implements the contract and extends the CLI. Read
...
registers through the shared `tai42_app` handle when the manifest loads it; no
