...
<Steps>
  <Step title="Install the runtime">
    ...
    five source repositories side by side and run `uv sync` in `tai-skeleton`
    (this quickstart runs the bundled `examples/hello`, so it needs the checkout,
    not just `pip install tai42-skeleton`).
  </Step>
