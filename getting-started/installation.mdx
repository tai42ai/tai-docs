...
`tai-skeleton`'s `[tool.uv.sources]` points at `../tai-contract` and
...
<Note>
Once the packages are published, `pip install tai-skeleton` replaces the
from-source flow. Until then, install from the side-by-side checkouts.
</Note>

## Add the standard toolbox

The `toolbox` extra pulls in the `tai-toolbox` contrib package — its generic
...
Once the packages are published, `pip install tai-skeleton[toolbox]` works too.
See the [standard toolbox guide](/guides/standard-toolbox) for what ships in it.

## The `tai` command

Installing the package puts one entrypoint on your path, `tai`. Three
subcommands run the long-lived processes:
