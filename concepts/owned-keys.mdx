...
- **Minted by an owner.** A non-admin mints only keys owned by itself; an admin
  may also mint a key for a **service principal** (see
  [accounts](/concepts/accounts)). Ownership is exactly one level deep — an owned
  key can never itself mint keys.
...
admins and editor/viewer role-holders — keep the full operator view (their limits
come from scopes and jq fences, not isolation). This single
...
same meaning, on both `ask` (interactions) and `notify_user` (notifications).
...
- **A restricted caller addresses only its own slice — it cannot broadcast.** The
  ...
  auto-scopes the send to that key itself: an unaddressed `notify_user` or `ask`
