...
of **`tool`** dispatches a registered tool **statelessly** — no thread, no memory,
...
An `agent` route's thread key is `bridge:{route_name}:{client_address}`, so each
client address holds its own conversation on a route and the agent remembers earlier
turns across separate inbound messages. Continuity is the checkpoint store's: with an
...
A `tool` target keeps no conversation: each message is dispatched on its own, with no
thread to remember or expire.
