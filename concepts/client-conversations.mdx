...
either a plain string (a text-only message) or an object carrying `message` with
optional `media`, tappable `options`, a pre-approved `template`, or an
...
several bubbles, attach an image, or offer tappable options. A part may be
**media-only** — an object with `media` and no text — for a caption-less image. The
array is strict: an unknown key or an empty array is a loud error, never silently
coerced.
...
An operator reply may itself carry **media**, tappable **options**, or an
[ask-less form](/concepts/notifications#ask-less-forms)'s **schema** — the same rich
content a produced answer can — validated up front so a bad attachment is a clean
rejection rather than a failed send. It is allowed in **either** mode and **never
flips the mode**.
