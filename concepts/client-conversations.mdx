...
- **`api`** — an authed caller `POST`s to
  ...
  HTTPS callback to the route's `callback_url` when it declares one, returned inline
  on a bounded sync wait, or read back from the poll door when it declares none.
...
An authed integration can also deliver a **structured event** into an existing thread
through the [event door](/reference/conversation-bridge#the-event-door): the event
enters that thread as a turn, ordered behind any in-flight message on the **same
per-thread FIFO** an inbound message takes, and its answer is delivered by the target
route's own door. An event carries no human text — it runs a `tool` target only — and
never mints a thread; it always joins one that already exists.
...
| `start_expr` / `reply_expr` / `cancel_expr` / `resume_expr` / `extras_expr` | Either target kind: jq mapping the message to the run's kwargs, the result to the reply, and driving the run's [parked interactions](/concepts/asking-the-caller). |
| `channel` / `our_identity` | `channel` rows: the registry name and the address texted. |
| `callback_url` | `api` rows, optional: the HTTPS answer sink; without one, answers are read from the poll door. |
| `overlap` | The [overlap policy](#when-messages-overlap): how a running turn treats newer messages on the same thread — leave it, cancel it, or carry a burst into one turn. The default leaves each message its own turn. |
...
one message in and one reply out. **Either** kind carries the route's optional jq
programs shaping the call:

- **`start_expr`** maps the inbound payload `{message, sender, our_identity,
  channel}` to the run's kwargs — a tool's kwargs, or an agent's `astream` kwargs;
  without it a tool gets `{message, sender}` and an agent reads the rendered turn text.
- **`reply_expr`** maps the run's result to the reply text; without it a tool result
  ...
  skips the mapping and resolves as the client-safe `error` outcome instead. It also
  binds `$turn`, `$asks`, and `$parked` (see [jq variables](/concepts/jq-variables#route-reply)).
- **`cancel_expr` / `resume_expr` / `extras_expr`** — the [door
  contract](/concepts/asking-the-caller#the-door-contract) driving the run's parked
  interactions, each reading `$parked`.
...
**answers** a pending `ask` its context rides the answer's `params`, and when the
...
sync-wait reply, or a poll-door record) whose `status` is `silent` and which carries no answer text. That
...
A `tool` target may also carry a
[`state_binding`](/concepts/states#a-bindings-subject-and-scope), so the route injects a bound
state's `input` values into the dispatch and applies its `update` afterward — a binding applies
to tool targets (a registered preset or a plain tool). An `agent` target takes no binding: a
route with `target_kind: agent` that carries a `state_binding` is refused at save (`400`).

A tool or flow target need not answer in one synchronous step either. Like an agent
target, its turn can async-park on an `ask` — pausing for a person's answer or
its expiry — and deliver its resumed reply back into the originating thread later,
mapped through the route's reply mapping.

A run may also [ask its caller](/concepts/asking-the-caller) (`to="caller"`) rather
than a person. When a turn asks its caller instead of finishing, its `reply_expr` sees
those asks in `$asks` and maps them to the reply, and the route's `resume_expr`
resolves them on the next visit — the [door
contract](/concepts/asking-the-caller#the-door-contract) in full.

### Structured participant messages

A participant message is always **text first**. When an inbound message carries a
...
key, present **only** when the inbound carried one, so an existing `start_expr`
...
The values are **untrusted participant data**: the platform bounds them as pure transport
(a serialized-size and nesting cap) and never validates them against the form's
schema — a submission is exactly as trustworthy as typed participant text, so a tool that
...
untrusted, transport-bounded participant data.
...
A conversation carries a **locale** — a BCP 47 language tag (e.g. `he-IL`) the platform's
[rendering layer](/concepts/storage-and-resources#locale-variants) resolves every template and
list format against. The caller states the language; the flow never selects one, and a `null`
locale means none is known (never a silent default).

- **`ConversationMessage.locale`** — the participant's per-message language hint the channel resolved
  from its native inbound, carried beside the text (and accepted on the API message door). It
  seeds a first-contact person's stored locale and, absent a stored override, is the turn's
  resolved locale.
- **the person's stored `locale`** — the operator override held on the [`Person`](/concepts/client-conversations)
  row, seeded from the channel at first contact and canonical in storage. Set or clear it through
  `PUT /api/conversations/persons/{person_id}/locale` (the same write grant that forgets a thread)
  or the CLI's [`tai conversations set-person-locale`](/reference/cli/conversations) / read it with
  `tai conversations get-person`. A later turn never clobbers a stored override.
- **the subject `locale`** — the locale captured onto each turn's subject, resolved by
  precedence: the person's stored locale (operator override or first-contact seed) wins, else the
  message's per-message hint, else `None`. It is what the renderer reads for every template and
  the `list_format` filter.
...
[ask-less form](/concepts/notifications#ask-less-forms)'s `schema`. A form part may also
carry the same per-send [`data` and `pages`](/concepts/interactions#prefilling-per-send-options-and-pages)
an ask does — `data.values` prefills top-level properties, `data.options` supplies a per-send
choice list, and `pages` lays the form out in titled steps — so a reply part can open its form
already filled in and stepped. `data`/`pages` ride ONLY a form part (a part carrying `schema`);
on any other part they are refused, and on a form part each is cross-checked against that part's
`schema` before delivery. The delivery
...
carrying the answer inline when the turn finishes in time (a route with a callback then
suppresses it so it never double-fires). A route with no `callback_url` records the answer
for the poll door, `GET /api/conversations/{route_name}/messages/{message_id}`.
...
- `merged` — this message's text was carried into the turn named by its `successor_id` under an
  [overlap policy](#when-messages-overlap) (`deliver: all`).
- `superseded` — this message was dropped in favour of the turn named by its `successor_id` (a
  running turn cancelled by a newer message, or a queued message overtaken by a newer one on a
  `cancel` route).

`merged` and `superseded` are answerless terminal outcomes that name the turn that took the
message's place through a `successor_id`. On a **channel** route they are terminal and send
nothing; on an **api** route they are delivered as a marker exactly as `silent` is — the
callback carries the `status` and `successor_id` with no answer text.

A turn's own outcome is orthogonal: `answered`, `error`, `silent`, `merged`, or `superseded`
— an `error` answer carries generic client-safe text, never an internal detail; `silent`,
`merged`, and `superseded` carry no answer text at all, and `merged`/`superseded` name their
`successor_id`. Delivery is retried
...
## When messages overlap

A participant does not wait their turn. A second message often lands while the turn on the
first is still running. By default the platform keeps each message its own turn, strictly
ordered — the second waits behind the first and then runs. A route's `overlap` policy chooses a
different behaviour, in three knobs — a settle window needs `deliver: all` or `running: cancel` — (the [bridge
reference](/reference/conversation-bridge#overlapping-messages) has the exact fields and how
each outcome is delivered):

- **Know / cancel** (`running`) — `continue` leaves a running turn alone; `cancel` cancels it
  cooperatively in favour of the newer message, which runs instead.
- **Carry** (`deliver`) — `one` runs a turn per message; `all` carries every message accepted
  since the last turn started into one turn, in order.
- **Settle** (`settle_seconds`) — hold a turn back a few seconds after its lead message so a
  burst rides one turn rather than several.

The policy is a generic conversation mechanism: it governs participant message turns only, and
the target — an agent or a tool — needs to know nothing about it. Under `deliver: all` the turn
simply sees the whole burst as its text (and, for a tool target, a `messages` list); under
`cancel` a turn cut short resolves `superseded` and delivers nothing, and its record names the
message that replaced it through `successor_id`.

### The handler can cooperate

Cancellation is **cooperative**: it can only stop a turn at an `await`, so it cannot claw back a
synchronous step already running, and it never reverts state the turn already wrote. Where that
matters, the handler cooperates — any code running inside a turn can read the **pending seam** to
learn a newer message is waiting and yield before an expensive or irreversible step:

- `current_conversation_turn()` returns the turn in flight (its thread and lead message), or
  `None` outside any turn.
- `app.conversations.pending_messages(thread_id, after=<the lead message id>)` returns the
  participant messages accepted after the lead and not yet carried into a turn.

A body that finds something pending stops early by raising `TurnSupersededError(successor_id=…)`
— the same outcome a cancel produces: no reply, no error, no delivery. `TurnSupersededError` is a
`BaseException`, not an `Exception`, so no broad `except Exception` between the yielding body and
the platform's turn runner — which catches it explicitly — ever swallows the yield. Under `running: continue`
this is how a handler implements "the newer message wins": it yields, the next turn runs, and
under `deliver: all` the yielded message rides that turn. The handler picks which steps must not
run past a newer message; the platform stays agnostic to what it does.
...
carrying the greeting text. A greeting owed on a turn that is then superseded — cancelled by
a newer message or yielded by the turn body — is never dropped: it is carried to the successor
turn and leads that turn's reply, on every door. Whether the suppressed inbound is **remembered** turns on
