...
[deep agents](/concepts/deep-agents). Its two implementations,
`langchain_deep_agent` and the [`claude_code`](/guides/use-the-ready-made-agents)
agent, run that durable, code-executing work over the
[sandbox](/concepts/sandbox) — the execution surface an agent reaches for a
persistent workspace and code.
