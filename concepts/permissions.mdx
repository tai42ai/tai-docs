...
**whether it is an admin** — the admin verdict is a condition-free `*` effective
policy, computed server-side and stamped on the request, never taken from a
provider's word.

| Identity | Admin? | Authenticates via | What it reaches |
|---|---|---|---|
| **Owner** | Admin | `POST /api/setup` → owner key | The first admin, created once by the setup door. Holds an admin key and, when an accounts provider is configured, an interactive login. |
| **Admin account** | Admin | password → session | A human on the reserved `admin` role — full reach, including access-control administration and every route with no mapping. Its per-tag pass is skipped. |
...
| **Service principal's key** | Non-admin | `sk-…` bearer | A key an admin minted for a non-human **service principal**. Reaches exactly the routes its scopes map to — the one identity the route→scope table meaningfully gates. |
| **Unauthenticated** | Public | — none — | No credential. Reaches only public routes: the `/api/login` sign-in surface, anything declared or pinned public (a health or readiness probe), and the dataless Studio SPA shell. |
