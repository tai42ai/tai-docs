...
[interaction](/concepts/interactions): where `ask` suspends the caller and
...
| `form` **question** delivery (`ask`)⁹ | Flow | web widget | webview of the callback form page | Block Kit modal | **declined** |
...
⁸ Telegram and Slack deliver a form **question** (`ask` with
...
same field, with the same isolation rules, that `ask` uses: a restricted
...
When to use which: a form **ask** (`ask` with `answer_format="form"`) suspends
...
- [Interactions](/concepts/interactions) — the blocking counterpart, `ask`, and
