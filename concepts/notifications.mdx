...
With a named channel, `media`, `template`, and `options` deliver only where the
channel advertises the matching capability; with no channel, they are stored on
the inbox record as given. `template` is the out-of-window send and stands alone: it is
...
So `media` reaches all five, `options` reach all but Twilio, and `template` is
WhatsApp-only.
