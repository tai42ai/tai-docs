...
`media` and `template` are mutually exclusive on one send, and each needs a named
channel — supplying either with no channel is a `400`. A channel that does not
advertise the capability rejects the send with `501 not supported` rather than
silently downgrading it to text; today only the
...
and `--template`, but **no `--audience`** — addressing a specific identity is
