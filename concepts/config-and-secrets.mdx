...
from the shared `TAI_DEFAULT_*`.

| Feature | Enabling variable(s) | OFF behavior |
| --- | --- | --- |
| Background tool runs | `TAI_TOOL_RUNS_REDIS_URL` | Run reads answer `200` empty (a single run reads `404`); submit refuses `501` `tool-runs-not-configured`. |
| Interactions + notifications | `INTERACTIONS_REDIS_URL` | `ask_user` and the in-app notification feed refuse `501` `interactions-not-configured`; feed reads answer `200` empty. |
| Rate limiting | `TAI_RATE_LIMIT_REDIS_URL` | Pass-through — the three public-door families are unthrottled; one boot `WARNING`. |
| Marketplace store | `MARKETPLACE_STORE_PG_PASSWORD` | Read-only mode: catalog reads answer `200` empty; install/attribution writes refuse `501` `marketplace-not-configured`. |
| Tool-metadata overlay | `TOOL_META_STORE_PG_PASSWORD` | Folder and tag reads answer `200` empty (`{"folders": [], "meta": []}`); writes refuse `501` `tool-meta-not-configured`. |
| Connectors store | `CONNECTOR_STORE_PG_PASSWORD` | Connector endpoints answer OFF; writes refuse `501` `connectors-not-configured`. |
| Versioning | `VERSIONING_STORE_PG_PASSWORD` | Preset and version reads answer `200` empty or `404`; writes refuse `501` `versioning-not-configured`. |
| Access control | `ACCESS_CONTROL_ENABLE` (with `ACCESS_CONTROL_REDIS_URL`, `ACCESS_CONTROL_STORE_PG_*`) | With the flag off, no gate runs on any request and the whole ecosystem is auth-free. |
