...
A sensitive value is referenced by the **name** of the environment variable that
holds it, not by value — the runtime reads it from the environment at use time, so a
file that is edited, versioned, or displayed never contains the credential itself.
The reference takes one of two shapes, by where it sits:

- **A scalar value written `!ENV ${VAR}`** is the convention wherever a secret rides
  as a plain string leaf — a [manifest](/concepts/manifest) value, a Studio secret
  field, and a [preset](/concepts/presets)'s baked `fixed_kwargs`. The runtime keeps
  the marker verbatim and resolves it from the environment at use time; the
  `!ENV ${VAR:default}` form falls back to `default` when the variable is absent. The
  Studio `SecretRefField` and MCP-tab secret flow write this marker for a pasted
  secret automatically (see [Settings profiles](#settings-profiles)).
- **A `secret_env` field** carries the env var name where the binding is a config
  object rather than a scalar — a hook or webhook verifier binding names `secret_env`,
  and an OAuth connector provider a `client_secret_env`.

Either way the credential stays out of every file that is edited, versioned, or
displayed.
...
- **Transactional writes, in both config modes.** A write holds exclusive access
  ...
  interleave and lose an update — the `file` provider takes a lock, the `kubernetes`
...
**Only `!ENV` references follow the profile.** A profile switches the
...
[referenced, not embedded](#secrets-are-referenced-not-embedded), stays masked, and
...
[Rotating `CONNECTORS_KEK`](#rotating-connectors_kek): rotate the key with a
...
| Interactions + notifications | `INTERACTIONS_REDIS_URL` | The notification feed and stream refuse 501 interactions-not-configured (ask refuses as a tool error); feed reads answer 200 empty. | `interactions` reports `off` |
...
| Subject state records | `TAI_DATABASE_DEFAULT_PG_PASSWORD` | Every states read and write refuses 501 states-not-configured. | `states` reports `off` |
...
| --- | --- | --- |
| `TAI_CONFIG_MODE` | `file` | Selects the config provider. `file` (default) reads from disk; `kubernetes` loads the Kubernetes config provider plugin. Any other value is **rejected loudly**. |
...
| Variable | Default | What it does |
| --- | --- | --- |
| `TAI_BUS_REDIS_URL` | unset | Redis URL that enables the bus. Unset = bus off; the process runs on a no-op local bus (valid only for a single worker, `file` mode, no backend). **Required** for a multi-worker server, a registered backend, or `TAI_CONFIG_MODE=kubernetes` — the boot rules refuse those shapes without it. |
...
| --- | --- |
| `CONNECTORS_KEK` | Base64 32-byte AES-GCM key encrypting connector token blobs. Required once connectors store credentials. Blobs are always **encrypted** under this current key. |
| `CONNECTORS_KEK_PREVIOUS` | Comma-separated base64 32-byte previous KEK(s), retained only so decrypt can still open blobs written under a superseded key while a rotation converges. Set during a rotation, emptied once it completes. |
...
1. **Carry the old key.** Set `CONNECTORS_KEK_PREVIOUS` to the current key's value and
   set `CONNECTORS_KEK` to the new key. Every stored blob still decrypts (via the ring),
   and every new/refreshed token is written under the new key.
2. **Converge the store.** Run `tai connectors reencrypt-tokens` (route
   `POST /api/connectors/tokens/reencrypt`). It scans every stored blob — expired
   sessions included — re-encrypts each one still under a previous key with the current
   key, and reports `{scanned, reencrypted, skipped, failed, failed_connection_ids,
   cas_retries}`. It is idempotent (a blob already under the current key is skipped) and
   compare-and-set safe (a concurrent token refresh is never clobbered). A blob no
   configured key can open is counted in `failed` and named — never silently dropped.
3. **Retire the old key.** Once the sweep reports `failed: 0`, clear
   `CONNECTORS_KEK_PREVIOUS`. The old key is no longer needed and its exposure window
   closes.

This is the controlled rotation path the key-material refusal points at (see
[Settings profiles](#settings-profiles) above, "Key material is refused, not
carried"): a KEK is rotated through these steps, never by carrying it in a settings
profile.
...
| `INTERACTIONS_BLOCKING_GRACE_SECONDS` | `5` | Outer bound a blocking `ask` waits past its own deadline. |
...
from the [Plugins section](/plugins). The platform documents the provider
