...
### Re-declaring and migrating

A plain re-declare that would **remove or change** a field while records exist is
refused (`409`): the change must go through the guarded migrate door. A migrate
that narrows the schema must supply exactly one of a transform or a drop
confirmation; supplying neither returns `412` with the narrowing message. A
migrate transform that fails, or produces a record invalid under the new schema,
aborts the whole change (`422`) — no partial state is ever committed.
...
| **Transfer tool** | door `transfer`; actor = the principal | an explicit subject map |
...
| `POST /api/states/{name}/migrate` | migrate the schema |
| `POST /api/states/{name}/migrate/preview` | preview a migrate |
...
`states-not-configured`), `404` not found, `409` exists / in use / conflict, `422`
validation (schema, subject, regime, path, value), `412` when a narrowing migrate
needs its drop confirmation.
...
`stats`, `migrate`, `migrate-preview`, `mounts`, `mount`, `update-mount`,
...
## Moving a flows-owned store onto the platform

A deployment whose flows engine kept its own state store moves the records onto the
platform store with the `flow_states_transfer` tool. It reads the flow-owned
declarations, modules, mounts, records, aliases, applied ops, and bindings, and
writes them onto the platform store under a subject map you supply
(`{<state>: {target_kind, target_name, kind}}`): declarations and modules are
re-declared, records are imported under the mapped subjects, and each binding's
flat-string `subject_expr` is rewritten into a typed `{kind, key}` subject. The
whole transfer is all-or-nothing — a state with records but no map entry, or a
`person` key that does not resolve, refuses before anything is written.

Run it `dry_run` first to see the counts, then for real; the re-run is idempotent
(a record already present with equal data is skipped, a differing one refuses).
Pass `drop_source` once the transfer is verified to drop the flow-owned tables. A
first-serve guard refuses to serve a bound flow until the transfer has run, so an
un-transferred deployment fails loudly rather than serving empty documents.
