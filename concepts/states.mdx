...
Replacing a live mount's `declarations` can retire part of a module while the state still
holds OPEN records shaped by the old declarations. A module author registers a **mount
reconciler** on the states facet with `register_mount_reconciler(reconciler)` — a pre-write
hook run inside every `mount` and `update_mount_declarations` write, after the validators and
before the write. It receives a `MountReconcileContext`: the `state` name, the module document,
the `operation` (`mount` | `update_declarations`), the `previous_declarations` (`None` on a
first mount) and `new_declarations`, the mount `options`, and a record door bound to the state.
The reconciler either **raises** a `ModuleValidationError` to refuse the mount (naming the
records the new declarations would orphan) or **writes resolutions** through the context's record
door and returns, letting the mount commit with those writes.

The `options` bag on the `PUT`/`PATCH` mount body (`MountBody.options`, `--options` on the CLI)
is a free-form, **per-operation** directive bag the reconciler reads to decide how to settle
open records against the new declarations — for example, whether to close records the new
declarations no longer cover. It is passed to the reconcilers for THIS mount only, **never
stored or served back**; any key outside `path`/`parameters`/`declarations`/`options` is refused.
...
| `GET /api/states/{name}/mounts/{module}` | read a mount (404 if not mounted) |
...
`stats`, `mounts`, `mount`, `get-mount`, `update-mount` (its `--options` carrying the
per-operation reconcile directives),
