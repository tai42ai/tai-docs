...
- **`update`** — `{description, purpose: "update", params, reads, writes, jq}`. `jq` runs over
  the record's attached subtree (`.` is the current data at the attach path) with the caller's
  argument bound as [`$input`](/concepts/jq-variables#template-jq) and `$parameters`/`$declarations`
  bound, and returns an ordered,
...
A binding applies its whole update set in one transaction — every engaged state's
updates land together, and if any update fails the run's writes roll back as a
unit. The atomicity is automatic; nothing configures it.
...
- **`composing`** — the path is built up by many writers, so a whole-path
  ...
  `set`. The refusal names the path and template, for example: `composing path
  ['items'] of template 'utilities' admits only keyed ops or an append set`.
...
an async `ask` and resumed later rejoins the same attribution.
...
A consumer that stages several writes and lands them together opens a **unit of
work** over the states facet, bound to the scope it owns. Every write set staged
against the unit runs through the same applier the persisted write path runs — the
guards, the path ops, the composing-shape rule, and the whole-document schema — and
is projected, not stored: a validation, composing-shape, or schema error raises at
stage time, exactly as it would on a committed write.

While the unit is open, a read of a subject the unit has staged is served from the
**projection** — the committed document overlaid with the scope's staged writes in
order, so a later step reads its own earlier writes — while every other scope still
sees only the committed document.

`commit` applies every staged batch onto the latest committed documents in one
transaction, all-or-nothing: any failing write rolls the whole commit back and
raises loudly, and nothing lands. It reports each op's ledger answer and any
**divergence** between the staged projection and the answer the commit recorded —
the guards and `op_id` idempotency are authoritative at commit, so a divergence
naming each staged-vs-committed mismatch is a visible, logged field, never swallowed.
`discard` drops the staging. A unit left open when its scope ends is discarded at
teardown and the discard is logged.

A **savepoint** nests inside a unit: writes staged in the child scope are kept on a
clean exit and dropped on an exception, rolling back only the child's writes and
never the parent's. Savepoints nest to any depth, so a nested run rolls back its own
writes within the enclosing unit.

The unit of work is a programmatic facet API for in-process consumers; it has no HTTP
route.
