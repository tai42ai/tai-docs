...
Manifest MCP tab each label a tool **Display Name (raw_name)**, with the raw name
...
edit surface away. The [flow editor](/babelfish) labels its own surfaces the same way, its
canvas node labels bare.
...
- **Plugin-declared badges** are code-owned. A plugin declares them on the tool
  itself (Babelfish's `flow_step` declares `flow`, for example), so they ship with
  the tool.
