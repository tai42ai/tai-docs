...
| `start_expr` | an object, or `null` | The kwargs the started target runs on; `null` starts nothing. No `start_expr` starts the door's own default kwargs. (A flows-engine [holder](/babelfish#calling-another-flow-cancel-resume-start) instead merges its `start_expr` result over the resolved node input, the expression's keys winning.) |
