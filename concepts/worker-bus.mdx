...
apply, a compiled-template-cache eviction — the bus carries that change to every
other worker sharing the manifest and collects a per-worker outcome, so the whole
fleet converges instead of one process drifting ahead of its siblings.
...
- **A registered backend.** A manifest that names a `backend_module` runs a
  ...
  refused too.
- **`TAI_CONFIG_MODE=kubernetes`.** The `kubernetes` config mode exists for multi-pod shared
  config, and a pod cannot see its own replica count, so `kubernetes` mode always requires
  the bus.

The refusal names `TAI_BUS_REDIS_URL`. A busless `kubernetes` boot is rejected on the
missing bus, not on a later kubeconfig error, because the `kubernetes` and workers checks
...
A change to stored template content evicts the compiled-template cache fleet-wide,
not just on the worker that made the change. A template write, a template delete,
a directory delete, and the manual `tai templates clear-cache` each broadcast a
...
- [Deploy](/operate/deploy) — the multi-worker and `kubernetes` deployment shapes the boot
  rules gate.
