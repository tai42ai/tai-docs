...
server. The site is a static bundle behind an nginx proxy that forwards `/api` to
a registry. Its hostname and that upstream are deployment configuration: the
container reads the upstream from the runtime variable `API_UPSTREAM`, and the
build-time client variables are documented in
[`tai-marketplace-web`](https://github.com/tai42ai/tai-marketplace-web#environment).
