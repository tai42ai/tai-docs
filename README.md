...
reference sections are generated from the source repositories so they cannot
drift from the code.
...
reference/             Generated reference (API, CLI, Python SDK, Studio SDK,
                       settings, catalog)
contributing.mdx       Pointer to each repository's CONTRIBUTING guide
...
aligned with the source repositories): `ruff check scripts/` and
...
regenerates the reference against the source repos' `main` HEAD and opens an
...
template each source repo copies into its own workflows to fire that hook.

Every source repository the reference is generated from ships the `notify-docs`
workflow, so a push to `main` in any of them fires the `regenerate` job. Two
...
`schedule` (cron `17 6 * * *`, ~06:17 UTC). Every path regenerates against the
source repos' `main` HEAD and opens the automated regeneration PR if anything
